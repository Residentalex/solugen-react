import { normalizarConfig, normalizarConfigRI, normalizarConfigVSNT, CAMPOS_DETALLE_LABELS, CAMPOS_DETALLE_RI_LABELS, CAMPOS_TICKET_LABELS_VSNT, } from './ticketPlantillaConfig';
// ===== Constantes ESC/POS =====
const ESC = '\x1B';
const GS = '\x1D';
const LF = '\n';
...
export function feed(n) { return ESC + 'd' + String.fromCharCode(n); }
 ...
 */
export function formatFechaCorta(val) {
    if (!val)
        return '--';
    try {
        const d = new Date(val);
...
export function formatTicketPOS(data, company, config) {
                ...
                for (const det of detalles) {
                    ...
                    for (const linea of zona.lineas) {
                        ...
                        if (anchoTabularFPV > 0) {
                            ...
                            let textoPad;
                            if (al === 'derecha')
                                textoPad = rightVisible(texto, width);
                            else if (al === 'centro')
                                textoPad = centerVisible(texto, width);
                            else
                                textoPad = texto;
                            const { antes, despues } = comandosFormato({ ...linea.formato, alineacion: undefined });
                            const formateado = antes.join('') + textoPad + despues.join('');
...
export function formatTicketReciboIngreso(data, company, config) {
 ...
 */
export function formatTicketVoucherVisanet(data, company, config) {
 ...
 */
export function escposQRCode(qrData) {
...
export function formatTicket(data, company, config, tipoDoc) {
    switch (tipoDoc) {
        case 'TICKET_POS':
