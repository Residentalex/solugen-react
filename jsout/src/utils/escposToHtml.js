 ...
 */
export function escposToHtml(raw) {
    const parts = [];
    let i = 0;
    let bold = false;
    ...
    function flushLine() {
        ...
        if (scaleV > 1 || scaleH > 1) {
            ...
            if (sx !== sy)
                style.push(`display:inline-block;transform:scaleX(${sx});transform-origin:left center`);
        }
        if (align === 'center')
        ...
        const inner = segments.map((s) => {
            const sStyle = [];
            if (s.bold)
                sStyle.push('font-weight:700;text-shadow:0 0 0.5px currentColor');
            if (s.condensed && scaleV <= 1 && scaleH <= 1)
                sStyle.push('font-size:11px;line-height:1.3');
            if (s.underline || s.underlineThick)
                sStyle.push('border-bottom:3px solid currentColor;display:inline');
