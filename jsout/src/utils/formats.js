...
export function formatDateRaw(val) {
...
export function getColorMonograma(diasCredito) {
...
export function truncateEmpaque(val, maxLen = 12) {
    if (!val || val.length <= maxLen)
        return val || '';
    return `...${val.slice(-maxLen)}`;
}
export function truncateText(val, maxLen = 50) {
    if (!val)
        return '';
