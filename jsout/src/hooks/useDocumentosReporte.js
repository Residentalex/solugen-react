...
export function useDocumentosReporte(config) {
    ...
    const handleImprimir = useCallback(async (selectedIds) => {
        if (data.length === 0 && (!selectedIds || selectedIds.length === 0))
            return;
        setLoadingPdf(true);
        try {
            let blob;
    ...
    const handleFechasChange = useCallback((nuevas) => {
        if (nuevas) {
            setFechas(nuevas);
        }
    }, []);
