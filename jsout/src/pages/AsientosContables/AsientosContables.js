...
const AsientosContables = () => {
    ...
    const { state, rangoDefault, puedeEditar, tipoDoc, actions } = useDocumentoListado({
        modulo: screenCode,
    ...
    useEffect(() => {
        ...
        pantallaApi.obtenerPantallasConEntidades(sucursalActiva).then((pantallas) => {
            const pantalla = pantallas.find((p) => p.codigo?.toUpperCase() === screenCode.toUpperCase());
    ...
    const columns = [
        {
            ...
            width: 180,
            fixed: 'left',
            render: (doc, record) => (_jsx(Link, { to: `/FAsientoContable/${record.id}`, className: "paces-doc-link", children: _jsx(Text, { strong: true, children: typeof doc === 'string' ? doc : doc?.codigo || doc?.nombre || JSON.stringify(doc) }) })),
        },
        {
            title: 'Fecha',
            dataIndex: 'fecha',
    ...
    return (_jsx(DocumentListadoLayout, { columns: columns, data: state.data, rowKey: "id", loading: state.loading, total: state.total, page: state.page, pageSize: state.pageSize, scrollX: 1080, selectedRowId: state.selectedRow?.id, loadingError: state.loadingError, errorMessage: "Error al cargar asientos contables", onRefresh: actions.handleRefresh, onRowClick: actions.handleRowClick, onPageChange: actions.goToPage, toolbarProps: {
            extraLeft: (_jsx(Select, { placeholder: "Tipo Documento", allowClear: true, showSearch: true, style: { minWidth: 250 }, value: tipoDoc, onChange: (val) => { actions.handleSetTipoDoc(val); }, options: documentos.map((d) => ({ value: d.codigo, label: `${d.codigo} - ${toTitleCase(d.nombre || '')}` })), size: "small", filterOption: (input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase()) })),
            showFiltros: true,
