...
import { Card, Table, Tabs, Tag, Button, Space, Row, Col, Grid, message, Form, Input, InputNumber, Select, DatePicker, Typography, Modal, Dropdown, Alert, Empty, } from 'antd';
import { DeleteOutlined, PlusOutlined, SearchOutlined, ClearOutlined, ExclamationCircleOutlined, EditOutlined, MoreOutlined, CalendarOutlined, HolderOutlined, } from '@ant-design/icons';
...
import CotizacionVentaGuide from './CotizacionVentaGuide';
const { Text } = Typography;
const { TextArea } = Input;
// ===== Cálculo de fila para Cotización (Precio × Cantidad, incluye impuestos en total) =====
...
const CotizacionVentaFormulario = () => {
    const { id } = useParams();
    ...
    const documentoConfig = useDocumentoConfig(sucursalActiva, "COT");
    ...
    const diasCreditoValue = Form.useWatch('diasCredito', form) ?? 0;
    const sinOC = true;
    ...
    // ===== Permisos y generación de asientos =====
    ...
    useEffect(() => {
        ...
        form.setFieldsValue({
            ...
            diasCredito: cloneData.diasCredito || 0,
            nota: cloneData.nota || '',
        });
        // Actualizar texto de búsqueda del concepto
    ...
    useEffect(() => {
        ...
        cotizacionVentaApi.obtenerPorId(sucursalActiva, parseInt(id))
            ...
            const full = {
                ...
                cliente: res.cliente || '',
                entidad: res.entidad || { nombre: '', codigo: '', identificacion: '' },
                moneda: res.moneda || getMonedaSucursalActiva(),
            ...
            // Cliente
            const clienteObj = {
                nombre: full.entidad?.nombre || full.cliente || '',
                codigo: full.entidad?.codigo || '',
                identificacion: full.entidad?.identificacion || '',
                telefono: full.entidad?.telefono || '',
                direccion: full.entidad?.direccion || '',
            };
                    ...
                    if (id) {
                        ...
                        cotizacionVentaApi.obtenerPorId(sucursalActiva, parseInt(id))
                            ...
                            const full = {
                                ...
                                estado: res.estado, periodo: res.periodo, ncf: res.ncf || '',
                                ncfModificado: res.ncfModificado || '', nota: res.nota || '',
                                referencia: res.referencia || '', tasa: res.tasa || 1, diasCredito: res.diasCredito || 0,
                                concepto: res.concepto || null, cliente: res.cliente || '',
                                entidad: res.entidad || { nombre: '', codigo: '', identificacion: '' },
                                moneda: res.moneda || getMonedaSucursalActiva(), documento: res.documento,
                                subTotal: res.subTotal, descuento: res.descuento, impuestos: res.impuestos, total: res.total,
                                almacen: res.almacen || null,
                                sucursal: res.sucursal || null, tipoDocumento: res.tipoDocumento,
                                detalles: (res.detalles || []).map((d) => ({
                            ...
                            const clienteObj = {
                                nombre: full.entidad?.nombre || full.cliente || '', codigo: full.entidad?.codigo || '',
                                identificacion: full.entidad?.identificacion || '', telefono: full.entidad?.telefono || '',
                                direccion: full.entidad?.direccion || '',
                            };
                            setSelectedCliente(clienteObj);
                            const fechaDoc = full.fechaDocumento ? parseDateRaw(full.fechaDocumento) : null;
                            form.setFieldsValue({
                                concepto: full.concepto?.codigo || '', cliente: full.entidad?.codigo || '',
                                almacen: full.almacen?.codigo || '',
                                fechaDocumento: fechaDoc ? dayjs(fechaDoc) : null,
                                ncf: full.ncf || '', referencia: full.referencia || '',
                                tasa: full.tasa || 1, diasCredito: full.diasCredito || 0, nota: full.nota || '',
                            });
                            if (full.concepto?.codigo) {
                                facturaPOSApi.obtenerClientes(sucursalActiva)
                                    .then((res) => setClientesCache(Array.isArray(res) ? res : []))
                            ...
                            .catch((err) => {
                            const msg = err?.response?.data?.errorMessage || 'Error al recargar la cotización';
                            message.error(msg);
                        })
                            .finally(() => setLoading(false));
                    }
    ...
    const validarFormulario = () => {
        ...
        // Almacén requerido solo si hay productos (no servicios)
        ...
        if (detalles.length === 0)
            return 'No se puede crear una cotización sin detalle.';
        ...
        // Validar según FechaPermitida del documento
    ...
    // ===== Loading state =====
    if (loading) {
        return _jsx(LoadingSpinner, { mensaje: "Cargando cotizaci\u00F3n..." });
    }
    // ===== Estado info =====
    ...
    const renderEncabezado = () => (_jsx(Card, { className: "paces-card", size: "small", title: "Datos Generales", extra: _jsx(EstadoTag, { estado: estado, periodo: data?.periodo }), style: { marginBottom: 16 }, children: _jsxs(Row, { gutter: 16, children: [_jsx(Col, { xs: 24, xxl: 18, children: _jsx(Form, { form: form, layout: "vertical", size: "middle", style: { paddingTop: 24 }, children: _jsxs(Row, { gutter: [16, 24], children: [_jsxs(Col, { xs: 24, children: [_jsxs("div", { ref: conceptoRef, children: [_jsx(FloatingField, { label: "Concepto", required: true, children: _jsx(Input, { placeholder: " ", value: selectedConcepto ? `${selectedConcepto.codigo || ''} - ${toTitleCase(selectedConcepto.nombre)}` : conceptoSearchText, readOnly: true, suffix: _jsxs(Space, { size: 4, children: [_jsx(SearchOutlined, { onClick: handleConceptoSearchClick, style: { cursor: 'pointer', color: 'rgba(0,0,0,0.45)' } }), selectedConcepto && _jsx(ClearOutlined, { onClick: handleConceptoClear, style: { cursor: 'pointer' } })] }), onClick: handleConceptoSearchClick }) }), _jsx(ConceptoInfoLabel, { concepto: selectedConcepto })] }), _jsx(Form.Item, { name: "concepto", hidden: true, children: _jsx(Input, {}) })] }), _jsx(Col, { xs: 24, sm: 12, lg: 9, children: _jsx(Form.Item, { name: "fechaDocumento", required: true, style: { marginBottom: 0 }, children: _jsx(FloatingField, { label: "Fecha Documento", required: true, children: _jsx(DatePicker, { style: { width: '100%' }, format: "YYYY-MM-DD", disabledDate: (current) => {
                                                    ...
                                                    // Si el cliente es exento de impuestos, limpiar
                                                    ...
                                                    setSelectedAlmacen(alm || null);
                                                        ...
                                                        } })) : tasaValue !== 1 ? (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('tasa'), children: ["Tasa: ", tasaValue, " ", _jsx(EditOutlined, {})] })) : (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('tasa'), children: [_jsx(PlusOutlined, {}), " Tasa"] })), editingField === 'diasCredito' ? (_jsx(InputNumber, { size: "small", style: { width: 120 }, min: 0, step: 1, placeholder: "D\u00EDas", autoFocus: true, defaultValue: editingValueRef.current, onChange: (val) => { editingValueRef.current = val ?? 0; }, onPressEnter: () => commitFieldEditor(), onBlur: () => commitFieldEditor(), onKeyDown: (e) => {
                                                            if (e.key === 'Escape') {
                                                                e.stopPropagation();
                                                                cancelFieldEditor();
                                                            }
                                                        } })) : diasCreditoValue ? (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('diasCredito'), children: ["Cr\u00E9dito: ", diasCreditoValue, "d ", _jsx(EditOutlined, {})] })) : (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('diasCredito'), children: [_jsx(PlusOutlined, {}), " Cr\u00E9dito"] }))] }) }), _jsx(Form.Item, { name: "ncf", hidden: true, children: _jsx(Input, {}) }), _jsx(Form.Item, { name: "referencia", hidden: true, children: _jsx(Input, {}) }), _jsx(Form.Item, { name: "tasa", hidden: true, children: _jsx(InputNumber, {}) }), _jsx(Form.Item, { name: "diasCredito", hidden: true, children: _jsx(InputNumber, {}) }), _jsx(Form.Item, { name: "moneda", hidden: true, children: _jsx(Input, {}) })] }), _jsx(Col, { xs: 24, children: _jsx(Form.Item, { name: "nota", style: { marginBottom: 0 }, children: _jsx(FloatingField, { label: "Nota", children: _jsx(TextArea, { rows: 3, maxLength: 500, showCount: true }) }) }) })] }) }) }), _jsx(Col, { xs: 24, xxl: 6, children: _jsx("div", { style: { marginTop: 24 }, children: _jsx(TotalesCard, { subTotal: totales.subTotal, descuento: totales.descuento, impuestos: totales.impuestos, total: totales.total, hideTitle: true, monedaSimbolo: data?.moneda?.simbolo || selectedConcepto?.moneda?.simbolo || monedaDefault.simbolo, monedaNombre: data?.moneda?.nombre || selectedConcepto?.moneda?.nombre || monedaDefault.nombre, tasa: tasaValue ?? data?.tasa ?? 1 }) }) })] }) }));
    return (_jsxs("div", { children: [_jsx(FormularioToolbar, { saving: saving, estado: estado, periodo: data?.periodo, onGuardar: handleGuardar, onCancelar: handleCancelar }), loadingError && (_jsx(Alert, { message: "Error al cargar formulario de cotizaci\u00F3n de venta", type: "error", showIcon: true, style: { marginBottom: 16 }, action: _jsx(Button, { size: "small", onClick: handleRefresh, children: "Reintentar" }) })), _jsx(BuscarConceptoModal, { open: conceptoModalOpen, onClose: () => setConceptoModalOpen(false), onSelect: handleConceptoSelect, sucursal: sucursalActiva, documento: "COT", tipoEntidad: "C" }), _jsx(BuscarProductoModal, { open: productoModalOpen, onClose: () => setProductoModalOpen(false), onSelect: handleProductoSelect, mode: "venta" }), _jsx(SeleccionarImpuestosModal, { open: modalImpuestosOpen, onClose: () => setModalImpuestosOpen(false), onConfirm: handleConfirmarImpuestos, tipoEntidad: "C", sucursal: sucursalActiva, existentes: impuestosFactura.map((i) => ({
                    codigo: i.codigo || '',
                    nombre: i.nombre || '',
                    ...
                    monto: i.monto,
                            ...
                            ...(data?.logs && data.logs.length > 0
                                ...
                                : []),
                        ] })] })), _jsx(ModalFechaVencimiento, { open: fechaVencimientoModal.open, onClose: () => setFechaVencimientoModal({ open: false, detalleId: 0 }), onFechaChange: handleFechaVencimiento }), (mode === 'crear' || esBorrador) && (_jsx(CotizacionVentaGuide, { mode: mode, concepto: selectedConcepto, almacen: selectedAlmacen, cliente: selectedCliente, detallesCount: detalles.length, tieneProductos: tieneProductos, conceptoRef: conceptoRef, almacenRef: almacenRef, clienteRef: clienteRef, agregarFilaRef: agregarFilaRef, sucursal: selectedSucursal, sucursalRef: sucursalRef }))] }));
};
