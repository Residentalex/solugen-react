...
const NotaCredito = ({ tipoEntidad }) => {
    ...
    const { state, rangoDefault, puedeEditar, actions } = useDocumentoListado({
        ...
        fetchVista: (sucursal, desde, hasta, filas, salto, estado) => notaCreditoApi.obtenerVista(sucursal, tipoEntidad, desde, hasta, filas, salto, estado),
        fetchFiltrar: (sucursal, params) => notaCreditoApi.filtrar(sucursal, tipoEntidad, {
    ...
    const handleClonar = async () => {
        if (!state.selectedRow)
            return;
        try {
            const data = await notaCreditoApi.obtenerPorId(sucursalActiva, state.selectedRow.id);
            const cloneData = {
                ...data,
                id: 0,
                noDocumento: '',
