...
const NotaDebito = ({ tipoEntidad }) => {
    ...
    const { state, rangoDefault, puedeEditar, actions } = useDocumentoListado({
        ...
        fetchVista: (sucursal, desde, hasta, filas, salto, estado) => notaDebitoApi.obtenerVista(sucursal, tipoEntidad, desde, hasta, filas, salto, estado),
        fetchFiltrar: (sucursal, params) => notaDebitoApi.filtrar(sucursal, tipoEntidad, {
    ...
    return (_jsx(DocumentListadoLayout, { columns: columns, data: state.data, rowKey: "id", loading: state.loading, total: state.total, page: state.page, pageSize: state.pageSize, scrollX: 1370, selectedRowId: state.selectedRow?.id, loadingError: state.loadingError, errorMessage: "Error al cargar notas d\u00E9bito", onRefresh: actions.handleRefresh, onRowClick: actions.handleRowClick, onPageChange: actions.goToPage, toolbarProps: {
            showFiltros: true,
