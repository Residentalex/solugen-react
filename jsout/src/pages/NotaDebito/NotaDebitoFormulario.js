...
import { Card, Table, Tabs, Tag, Button, Space, Row, Col, Divider, Grid, message, Form, Input, InputNumber, Select, DatePicker, Typography, Modal, Alert, Switch, Empty, } from 'antd';
...
function calcularTotales(impuestos, total, perdidas = 0) {
...
const TotalesSection = React.memo(({ impuestosRetenciones, montoTotal, perdidas = 0, monedaSimbolo, monedaNombre, tasa }) => {
    const totales = calcularTotales(impuestosRetenciones, Number(montoTotal) || 0, perdidas);
    return (_jsxs("div", { style: { marginTop: 24 }, children: [_jsx(TotalesCard, { subTotal: totales.subTotal, descuento: 0, impuestos: totales.impuestos, total: Number(montoTotal) || 0, hideTitle: true, monedaSimbolo: monedaSimbolo, monedaNombre: monedaNombre, tasa: tasa }), perdidas > 0 && (_jsx("div", { style: { marginTop: 8, textAlign: 'right' }, children: _jsxs(Text, { type: "warning", strong: true, children: ["P\u00E9rdida: ", formatCurrency(perdidas)] }) }))] }));
});
const NotaDebitoFormulario = ({ tipoEntidad }) => {
    const { id } = useParams();
    ...
    const [impuestosRetenciones, setImpuestosRetenciones] = useState([]);
    ...
    const [montoTotalConfirmado, setMontoTotalConfirmado] = useState(0);
    // NCF
    ...
    const totales = calcularTotales(impuestosRetenciones, Number(form.getFieldValue('total')) || 0, devoluciones.reduce((s, d) => s + (d.perdida || 0), 0));
    // ===== Quick field editors =====
    ...
    const commitFieldEditor = () => {
        ...
        if (field) {
            ...
            if (field === 'ncfModificado') {
                setNcfModificadoVal(editingValueRef.current);
            }
        }
    ...
    useEffect(() => {
        ...
        if (cloneData) {
            ...
            if (entidadCloneNorm) {
                setEntidadesCache([entidadCloneNorm]);
            ...
            if (sucursalCloneNorm) {
                setSucursalesCache(prev => {
                    const existe = prev.find((x) => x.codigo === sucursalCloneNorm.codigo || x.idExterno === sucursalCloneNorm.codigo);
                    if (existe)
                        return prev;
                    return [...prev, sucursalCloneNorm];
                });
            ...
            const docsPagoClone = todasAsociadasClone.filter((x) => !x.esDocumentoInventario);
            const docsInventarioClone = todasAsociadasClone.filter((x) => x.esDocumentoInventario);
            const devsMapeadasClone = docsInventarioClone.map((x) => ({
            ...
            setImpuestosRetenciones((cloneData.impuestosFactura || []).map((imp) => ({
    ...
    useEffect(() => {
        ...
        notaDebitoApi.obtenerPorId(sucursalActiva, parseInt(id))
            ...
            setEntidadesCache(entidadNormalizada ? [entidadNormalizada] : []);
            ...
            const docsPago = todasAsociadas
                .filter((x) => !x.esDocumentoInventario)
                .map((x) => ({ ...x, nCF: x.nCF || x.ncf || '' }));
            const docsInventario = todasAsociadas.filter((x) => x.esDocumentoInventario);
            const devsMapeadas = docsInventario.map((x) => ({
                transaccionAsociadaID: x.transaccionAsociadaID || x.id,
                documento: x.documento,
                fecha: x.fecha,
                montoOriginal: x.montoOriginal,
            ...
            setImpuestosRetenciones((full.impuestosFactura || []).map((imp) => ({
                codigo: imp.impuesto?.codigo,
                idExterno: imp.impuesto?.idExterno,
                nombre: imp.impuesto?.nombre,
                porcentaje: imp.impuesto?.porcentaje,
            ...
            if (sucursalData) {
                setSucursalesCache(prev => {
                    const existe = prev.find((x) => x.codigo === sucursalData.codigo || x.idExterno === sucursalData.codigo);
                    if (existe)
                        return prev;
                    return [...prev, sucursalData];
                });
            ...
            if (full.concepto?.codigo) {
                entidadApi.obtenerActivos(sucursalActiva, full.concepto.codigo, tipoEntidad)
                    ...
                    if (full.entidad && !ents.find((e) => e.codigo === entidadCodigo)) {
                        return setEntidadesCache([entidadNormalizada, ...ents]);
                    }
                    setEntidadesCache(ents);
                })
                    .catch(() => {
                    if (full.entidad) {
                        setEntidadesCache([full.entidad]);
                    }
                });
            }
        })
    ...
    const handleDocMontoChange = (id, monto) => {
        setDocumentosRelacionados((prev) => prev.map((d) => (d.transaccionAsociadaID === id || d.id === id) ? { ...d, monto } : d));
    };
    // ===== Handlers de Devoluciones =====
    const handleDevolucionSelect = (docs) => {
        setDevoluciones((prev) => {
            const existentes = new Set(prev.map((d) => d.transaccionAsociadaID));
            const nuevos = docs
                .filter((d) => !existentes.has(d.transaccionAsociadaID))
                .map((d) => ({
                transaccionAsociadaID: d.transaccionAsociadaID,
                documento: d.documento,
                fecha: d.fecha,
            ...
            if (noDoc) {
                devolucionCompraApi.obtenerPorNoDocumento(sucursalActiva, noDoc)
                    ...
                    if (devolucion?.detalles && (!tipoCalienteCodigo || devolucion?.tipo?.codigo === tipoCalienteCodigo)) {
                        ...
                        devolucion.detalles
                            .filter((det) => det.impuesto != null && det.impuestos > 0)
                            .forEach((det) => {
    ...
    const handleDevolucionRemove = (id) => {
        setDevoluciones((prev) => prev.filter((d) => d.transaccionAsociadaID !== id));
    };
    const handleDevMontoChange = (id, monto) => {
        setDevoluciones((prev) => prev.map((d) => d.transaccionAsociadaID === id
            ? { ...d, monto }
            : d));
    };
    // ===== Handlers de Impuestos y Retenciones =====
    const handleImpuestoRemove = (id) => {
        setImpuestosRetenciones((prev) => prev.filter((i) => i.id !== id && i.codigo !== id));
    };
    const handleImpuestoChange = (id, field, value) => {
        setImpuestosRetenciones((prev) => prev.map((i) => (i.id === id || i.codigo === id) ? { ...i, [field]: value } : i));
    };
    ...
    const validarFormulario = async () => {
        ...
        if (!values.entidad && !selectedEntidad)
            return `El ${entidadLabel} es obligatorio`;
        // Fecha ≤ hoy
    ...
    const construirDTO = () => {
        ...
        const docConfig = documentos.find((d) => d.codigo === 'ND');
        ...
        return {
            ...
            transaccionesAsociadas: [...documentosRelacionados, ...devoluciones.map((d) => ({
                    transaccionAsociadaID: d.transaccionAsociadaID,
                    documento: d.documento,
                    fecha: d.fecha,
                    montoOriginal: d.montoOriginal,
    ...
    const handleRefresh = useCallback(() => {
        ...
        notaDebitoApi.obtenerPorId(sucursalActiva, parseInt(id))
            ...
            const docsPagoRefresh = todasAsociadasRefresh
                .filter((x) => !x.esDocumentoInventario)
                .map((x) => ({ ...x, nCF: x.nCF || x.ncf || '' }));
            const docsInventarioRefresh = todasAsociadasRefresh.filter((x) => x.esDocumentoInventario);
            const devsMapeadasRefresh = docsInventarioRefresh.map((x) => ({
                transaccionAsociadaID: x.transaccionAsociadaID || x.id,
                documento: x.documento,
                fecha: x.fecha,
                montoOriginal: x.montoOriginal,
            ...
            if (sucursalRefreshData) {
                setSucursalesCache(prev => {
                    const existe = prev.find((x) => x.codigo === sucursalRefreshData.codigo || x.idExterno === sucursalRefreshData.codigo);
                    if (existe)
                        return prev;
                    return [...prev, sucursalRefreshData];
                });
    ...
    const docRelColumns = [
        ...
        { title: 'Pendiente', key: 'pendienteCalc', width: 120, align: 'right', render: (_, record) => _jsx("strong", { children: formatNumber(Math.max(0, Math.round(((record.montoOriginal || 0) - (record.pagado || 0)) * 100) / 100)) }) },
        {
            title: 'Monto a Debitar', dataIndex: 'monto', key: 'monto', width: 140, align: 'right',
            render: (_, record, idx) => {
                ...
                return (_jsx(InputNumber, { size: "small", style: { width: 120 }, min: 0, max: Math.max(0, Math.round(((record.montoOriginal || 0) - (record.pagado || 0)) * 100) / 100), step: 0.01, precision: 2, value: documentosRelacionados[idx]?.monto, onChange: (val) => handleDocMontoChange(record.transaccionAsociadaID || record.id, val || 0) }));
            },
        },
        {
    ...
    const devColumns = [
        ...
        { title: 'Documento', dataIndex: 'documento', key: 'documento', width: 140 },
        {
            title: 'Monto Original', dataIndex: 'montoOriginal', key: 'montoOriginal', width: 140, align: 'right',
            render: (_, record) => formatNumber((record.montoOriginal || 0) + (record.impuesto || 0)),
        },
        {
            title: 'Pagado',
            dataIndex: 'pagado',
        ...
        },
        {
            ...
            width: 120,
            align: 'right',
            render: (_, record) => _jsx("strong", { children: formatNumber(Math.max(0, Math.round(((record.montoOriginal || 0) - (record.pagado || 0)) * 100) / 100)) }),
        },
        {
            title: 'Monto a Debitar', dataIndex: 'monto', key: 'monto', width: 140, align: 'right',
            render: (_, record, idx) => (_jsx(InputNumber, { size: "small", style: { width: 120 }, min: 0, step: 0.01, precision: 2, value: devoluciones[idx]?.monto, onChange: (val) => handleDevMontoChange(record.transaccionAsociadaID, val || 0) })),
        },
        {
            title: 'Pérdida',
            dataIndex: 'perdida',
            ...
            render: (v) => {
                const val = v ?? 0;
                if (val === 0)
                    return _jsx(Text, { style: { color: '#8c8c8c' }, children: "-" });
                return (_jsxs(Text, { style: { color: val > 0 ? '#ff4d4f' : '#52c41a', fontWeight: 600 }, children: [val > 0 ? '− ' : '+ ', formatNumber(Math.abs(val))] }));
            },
        },
        {
            title: 'Gen. Pérdida',
            ...
            key: 'generarPerdida',
            width: 110,
            render: (_, record, idx) => (_jsx(Switch, { size: "small", checked: record.generarPerdida, onChange: (checked) => {
        ...
        {
            title: '', key: 'accion', width: 50,
            render: (_, record) => (_jsx(Button, { type: "text", danger: true, size: "small", icon: _jsx(DeleteOutlined, {}), onClick: () => handleDevolucionRemove(record.transaccionAsociadaID) })),
        },
    ];
    const impuestoColumns = [
        ...
        },
        {
            ...
            width: 140,
            align: 'right',
            render: (_, record, idx) => (_jsx(InputNumber, { size: "small", style: { width: 120 }, min: 0, step: 0.01, precision: 2, value: impuestosRetenciones[idx]?.monto, onChange: (val) => handleImpuestoChange(record.id ?? record.codigo, 'monto', val || 0) })),
        },
        {
            title: '',
            key: 'accion',
            width: 50,
            render: (_, record) => (_jsx(Button, { type: "text", danger: true, size: "small", icon: _jsx(DeleteOutlined, {}), onClick: () => handleImpuestoRemove(record.id ?? record.codigo) })),
        },
    ];
    ...
    const detalleMovimientoColumns = [
        ...
        { title: 'Cantidad', dataIndex: 'cantidad', key: 'cantidad', width: 100, align: 'right',
            render: (v) => formatNumber(v || 0) },
    ...
    const renderEncabezado = () => (_jsx(Card, { className: "paces-card", size: "small", title: "Datos Generales", extra: _jsx(EstadoTag, { estado: estado, periodo: data?.periodo }), style: { marginBottom: 16, paddingBottom: 32 }, children: _jsxs(Row, { gutter: 16, children: [_jsx(Col, { xs: 24, xxl: 18, children: _jsx(Form, { form: form, layout: "vertical", size: "middle", style: { paddingTop: 24 }, children: _jsxs(Row, { gutter: [16, 24], children: [_jsx(Col, { xs: 24, sm: 12, lg: 6, children: _jsx(Form.Item, { name: "fechaDocumento", required: true, style: { marginBottom: 0 }, children: _jsx(FloatingField, { label: "Fecha", required: true, children: _jsx(DatePicker, { style: { width: '100%' }, format: "YYYY-MM-DD", disabledDate: (current) => {
                                                    ...
                                                    if (cierreInv && !current.isAfter(dayjs(cierreInv).startOf('day'), 'day'))
                                                        return true;
                                                    return false;
                                                } }) }) }) }), _jsxs(Col, { xs: 24, sm: 12, lg: 18, children: [_jsx("div", { ref: conceptoRef, children: _jsx(FloatingField, { label: "Concepto", required: true, children: _jsx(Input, { placeholder: " ", value: selectedConcepto ? `${selectedConcepto.codigo} - ${toTitleCase(selectedConcepto.nombre)}` : '', readOnly: true, disabled: documentoTieneTipos && !selectedTipo, suffix: _jsxs(Space, { size: 4, children: [_jsx(SearchOutlined, { onClick: () => (!documentoTieneTipos || selectedTipo) && setConceptoModalOpen(true), style: { cursor: (!documentoTieneTipos || selectedTipo) ? 'pointer' : 'not-allowed', color: 'rgba(0,0,0,0.45)' } }), selectedConcepto && _jsx(ClearOutlined, { onClick: handleConceptoClear, style: { cursor: 'pointer' } })] }), onClick: () => (!documentoTieneTipos || selectedTipo) && setConceptoModalOpen(true) }) }) }), _jsx(Form.Item, { name: "concepto", hidden: true, children: _jsx(Input, {}) }), _jsx(ConceptoInfoLabel, { concepto: selectedConcepto })] }), _jsxs(Col, { xs: 24, sm: 12, lg: 6, children: [_jsx("div", { ref: tipoRef, children: _jsx(FloatingField, { label: "Tipo", required: true, children: _jsx(Input, { placeholder: " ", value: selectedTipo ? `${selectedTipo.codigo} - ${toTitleCase(selectedTipo.nombre)}` : '', readOnly: true, suffix: _jsxs(Space, { size: 4, children: [_jsx(SearchOutlined, { onClick: () => setTipoModalOpen(true), style: { cursor: 'pointer', color: 'rgba(0,0,0,0.45)' } }), selectedTipo && _jsx(ClearOutlined, { onClick: handleTipoClear, style: { cursor: 'pointer' } })] }), onClick: () => setTipoModalOpen(true) }) }) }), _jsx(Form.Item, { name: "tipo", hidden: true, children: _jsx(Input, {}) })] }), _jsx(Col, { xs: 24, sm: 12, lg: 18, children: _jsx("div", { ref: entidadRef, children: _jsx(Form.Item, { name: "entidad", required: true, style: { marginBottom: 0 }, children: _jsx(BuscarEntidadSelect, { entidades: entidadesCache, value: selectedEntidad?.codigo, label: entidadLabel, required: true, tieneDocumentosAsociados: documentosRelacionados.length > 0 || devoluciones.length > 0, conceptoSeleccionado: !!selectedConcepto, onChange: (codigo, entidad) => {
                                                    ...
                                                    setMontoTotalConfirmado(Number(val));
                                                        ...
                                                        } })) : ncfModificadoVal ? (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('ncfModificado'), children: ["NCF Mod: ", ncfModificadoVal, " ", _jsx(EditOutlined, {})] })) : (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => { setNcfTipo('modificado'); openFieldEditor('ncfModificado'); }, children: [_jsx(PlusOutlined, {}), " NCF Mod"] })) }), editingField === 'referencia' ? (_jsx(Input, { size: "small", style: { width: 200 }, placeholder: "Referencia", autoFocus: true, defaultValue: editingValueRef.current, onChange: (e) => { editingValueRef.current = e.target.value; }, onPressEnter: () => commitFieldEditor(), onBlur: () => commitFieldEditor(), onKeyDown: (e) => {
                                                        if (e.key === 'Escape') {
                                                            e.stopPropagation();
                                                            cancelFieldEditor();
                                                        }
                                                    ...
                                                    } })) : tasaValue !== 1 ? (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('tasa'), children: ["Tasa: ", tasaValue, " ", _jsx(EditOutlined, {})] })) : (_jsxs(Tag, { style: { cursor: 'pointer', fontSize: 14 }, onClick: () => openFieldEditor('tasa'), children: [_jsx(PlusOutlined, {}), " Tasa"] }))] }), _jsx(Form.Item, { name: "ncf", hidden: true, children: _jsx(Input, {}) }), _jsx(Form.Item, { name: "referencia", hidden: true, children: _jsx(Input, {}) }), _jsx(Form.Item, { name: "tasa", hidden: true, children: _jsx(InputNumber, {}) })] }), _jsx(Form.Item, { name: "moneda", hidden: true, children: _jsx(Input, {}) })] }) }) }), _jsx(Col, { xs: 24, xxl: 6, children: _jsx(TotalesSection, { impuestosRetenciones: impuestosRetenciones, montoTotal: montoTotalConfirmado, perdidas: devoluciones.reduce((s, d) => s + (d.perdida || 0), 0), monedaSimbolo: data?.moneda?.simbolo || selectedConcepto?.moneda?.simbolo || getMonedaSucursalActiva().simbolo, monedaNombre: data?.moneda?.nombre || selectedConcepto?.moneda?.nombre || getMonedaSucursalActiva().nombre, tasa: tasaValue ?? data?.tasa ?? 1 }) })] }) }));
    // ===== Layout principal =====
    const tabItems = [
        {
            key: 'documentos',
            label: `Documentos (${documentosRelacionados.length})`,
            children: (_jsxs("div", { ref: documentosRef, children: [_jsx("div", { style: { marginBottom: 8 }, children: _jsx(Button, { type: "dashed", icon: _jsx(PlusOutlined, {}), onClick: () => setBuscarDocModalOpen(true), children: "Agregar Documento" }) }), _jsx(Table, { dataSource: documentosRelacionados, columns: docRelColumns, rowKey: (r) => r.transaccionAsociadaID || r.id || 0, size: "small", pagination: false, scroll: { x: 950 } })] })),
        },
        // Tab 2: Artículos (solo CLI)
        ...(tipoEntidad === 'CLI' ? [{
                key: 'articulos',
                label: `Artículos (${detallesMovimiento.length})`,
                children: (_jsxs("div", { children: [_jsx("div", { style: { marginBottom: 8, display: 'flex', justifyContent: 'flex-start' }, children: _jsx(Button, { type: "dashed", icon: _jsx(PlusOutlined, {}), onClick: () => {
                                    const nuevoId = -(detallesMovimiento.length + 1);
                                    setDetallesMovimiento(prev => [...prev, { id: nuevoId, codigo: '', articulo: '', cantidad: 0, total: 0 }]);
                                }, children: "Agregar fila" }) }), _jsx(Table, { dataSource: detallesMovimiento, columns: detalleMovimientoColumns, rowKey: "id", size: "small", pagination: false, scroll: { x: 600 }, locale: {
        ...
        ...(tipoEntidad === 'SUP' ? [{
                ...
                children: (_jsxs(_Fragment, { children: [_jsx("div", { style: { marginBottom: 8 }, children: _jsx(Button, { type: "dashed", icon: _jsx(PlusOutlined, {}), onClick: () => setBuscarDevModalOpen(true), children: "Agregar Devoluci\u00F3n" }) }), _jsx(Table, { dataSource: devoluciones, columns: devColumns, rowKey: (r) => r.transaccionAsociadaID || 0, size: "small", pagination: false, scroll: { x: 900 } })] })),
            }] : []),
        {
            key: 'impuestos',
            label: `Impuestos y Retenciones (${impuestosRetenciones.length})`,
            children: (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: 8 }, children: [_jsx(Button, { type: "primary", ghost: true, icon: _jsx(SearchOutlined, {}), onClick: () => setModalImpuestosOpen(true), children: "Seleccionar del cat\u00E1logo" }), impuestosRetenciones.length > 0 && (_jsx(Button, { type: "link", danger: true, style: { marginLeft: 8 }, onClick: () => setImpuestosRetenciones([]), children: "Limpiar todos" }))] }), _jsx(Table, { dataSource: impuestosRetenciones, columns: impuestoColumns, rowKey: (r) => r.id || r.codigo || 0, size: "small", pagination: false, scroll: { x: 600 }, locale: { emptyText: 'Sin impuestos seleccionados' } }), _jsx("div", { style: { marginTop: 8, display: 'flex', justifyContent: 'flex-end', gap: 16 }, children: (() => {
                            const totales = calcularTotales(impuestosRetenciones, Number(montoTotalConfirmado) || 0);
                            return (_jsxs(_Fragment, { children: [_jsxs(Text, { className: "paces-text-secondary", children: ["SubTotal: ", _jsx("strong", { children: formatNumber(totales.subTotal) })] }), _jsxs(Text, { className: "paces-text-secondary", children: ["Impuestos: ", _jsx("strong", { children: formatNumber(totales.impuestos) })] }), _jsxs(Text, { className: "paces-text-secondary", children: ["Retenciones: ", _jsx("strong", { children: formatNumber(totales.retenciones) })] })] }));
                        })() })] })),
        },
        {
            key: 'asientos',
    ...
    const contenidoPestanas = (_jsx(Tabs, { defaultActiveKey: "documentos", type: "card", items: tabItems }));
    return (_jsxs("div", { children: [_jsx(FormularioToolbar, { saving: saving, estado: estado, periodo: data?.periodo, onGuardar: handleGuardar, onCancelar: handleCancelar }), loadingError && (_jsx(Alert, { message: "Error al cargar formulario de nota de d\u00E9bito", type: "error", showIcon: true, style: { marginBottom: 16 }, action: _jsx(Button, { size: "small", onClick: handleRefresh, children: "Reintentar" }) })), isLarge ? (_jsx(Row, { gutter: 16, children: _jsxs(Col, { xxl: 24, children: [renderEncabezado(), contenidoPestanas] }) })) : (_jsxs("div", { children: [renderEncabezado(), contenidoPestanas] })), _jsx(BuscarConceptoModal, { open: conceptoModalOpen, onClose: () => setConceptoModalOpen(false), onSelect: handleConceptoSelect, fetchConceptos: () => conceptosApi.obtenerConceptosPorDocumento(sucursalActiva, 'ND'), sucursal: sucursalActiva, documento: "ND", tipo: selectedTipo?.codigo, tipoEntidad: tipoEntidad }), _jsx(BuscarTipoModal, { open: tipoModalOpen, onClose: () => setTipoModalOpen(false), onSelect: handleTipoSelect, tipoDocumento: "ND" }), _jsx(BuscarDocumentoModal, { open: buscarDocModalOpen, onClose: () => setBuscarDocModalOpen(false), onSelect: handleDocRelacionadoSelect, tipoEntidad: tipoEntidad, codEntidad: selectedEntidad?.codigo || '', origen: (() => {
                    const { documentos } = useCompanyStore.getState().data;
                    const docConfig = documentos.find((d) => d.codigo === 'ND');
                    return normalizarOrigen(docConfig?.origenCuenta ?? OrigenCuenta.Desconocido);
                })(), montoTotal: Number(form.getFieldValue('total')) || 0 }), tipoEntidad === 'SUP' && (_jsx(BuscarDocumentoModal, { open: buscarDevModalOpen, onClose: () => setBuscarDevModalOpen(false), onSelect: handleDevolucionSelect, tipoEntidad: tipoEntidad, codEntidad: selectedEntidad?.codigo || '', origen: (() => {
                    const { documentos } = useCompanyStore.getState().data;
                    const docConfig = documentos.find((d) => d.codigo === 'ND');
                    return normalizarOrigen(docConfig?.origenCuenta ?? OrigenCuenta.Desconocido);
                })(), esDocumentoInventario: true, montoTotal: Number(form.getFieldValue('total')) || 0 })), _jsx(SeleccionarImpuestosModal, { open: modalImpuestosOpen, onClose: () => setModalImpuestosOpen(false), onConfirm: handleConfirmarImpuestos, tipoEntidad: tipoEntidad, sucursal: sucursalActiva, existentes: impuestosRetenciones.map((i) => ({
                    codigo: i.codigo || '',
                    nombre: i.nombre || '',
                ...
                }, sucursal: sucursalActiva }), (mode === 'crear' || esBorrador) && (_jsx(NotaDebitoGuide, { mode: mode, concepto: selectedConcepto, sucursal: selectedSucursal, tipo: selectedTipo, entidad: selectedEntidad, detallesCount: documentosRelacionados.length + devoluciones.length, conceptoRef: conceptoRef, sucursalRef: sucursalRef, tipoRef: tipoRef, entidadRef: entidadRef, documentosRef: documentosRef }))] }));
};
