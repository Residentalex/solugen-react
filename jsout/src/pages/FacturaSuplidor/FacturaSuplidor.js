...
const FacturaSuplidor = () => {
    const navigate = useNavigate();
    const { screenCode, documentCode } = useScreenConfig('FRDE');
    const { state, rangoDefault, puedeEditar, actions } = useDocumentoListado({
        modulo: screenCode,
        fetchVista: (sucursal, desde, hasta, filas, salto, estado) => facturaSuplidorApi.obtenerVista(sucursal, desde, hasta, filas, salto, estado),
        fetchFiltrar: (sucursal, params) => facturaSuplidorApi.filtrar(sucursal, params),
    ...
    const handleClonar = async () => {
        if (!state.selectedRow)
            return;
        try {
            const data = await facturaSuplidorApi.obtenerPorId(sucursalActiva, state.selectedRow.id);
            const cloneData = {
                ...data,
                id: 0,
                noDocumento: '',
            ...
            navigate('/FRDE/nuevo', { state: { cloneData } });
        }
        catch (err) {
    ...
    const columns = [
        {
            ...
            width: 180,
            fixed: 'left',
            render: (doc, record) => (_jsx(Link, { to: `/FRDE/${record.id}`, className: "paces-doc-link", children: _jsx(Text, { strong: true, children: doc }) })),
        },
        {
            title: 'Fecha',
            dataIndex: 'fecha',
    ...
    return (_jsx(DocumentListadoLayout, { columns: columns, data: state.data, rowKey: "id", loading: state.loading, total: state.total, page: state.page, pageSize: state.pageSize, scrollX: 1370, selectedRowId: state.selectedRow?.id, loadingError: state.loadingError, errorMessage: "Error al cargar facturas de suplidor", onRefresh: actions.handleRefresh, onRowClick: actions.handleRowClick, onPageChange: actions.goToPage, toolbarProps: {
            showFiltros: true,
