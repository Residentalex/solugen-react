...
const TransaccionNoCuadrada = () => {
        ...
        try {
            ...
            if (sucursalFiltro === -1) {
                const sucursalesIds = (sucursalesDisponibles || [])
                    ...
                    .map((s) => s.sucursal);
                const resultados = await Promise.all(sucursalesIds.map(suc => transaccionApi.obtenerNoCuadrados(suc, desde, hasta, tipoDoc || undefined)
                    .catch(() => [])));
    ...
    useEffect(() => {
        setActiveModule('RTransNoCuadrada');
        updateToolbar({});
        return () => resetToolbar();
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 120,
            align: 'right',
            render: (v) => _jsx(Text, { children: formatCurrency(v) }),
        },
        {
            title: 'Créditos',
            dataIndex: 'creditos',
        ...
        },
        {
            ...
            align: 'right',
            render: (_, record) => {
                const diff = Math.abs((record.debitos || 0) - (record.creditos || 0));
                return _jsx(Text, { strong: true, style: { color: '#E05252' }, children: formatCurrency(diff) });
            },
        },
        {
            title: 'Estado',
    ...
    return (_jsx(DocumentListadoLayout, { columns: columns, data: filteredData, rowKey: "id", loading: loading, total: filteredData.length, page: page, pageSize: pageSize, scrollX: 1250, selectedRowId: selectedRow?.id, loadingError: loadingError, errorMessage: "Error al cargar asientos no cuadrados", onRefresh: handleRefresh, onRowClick: handleRowClick, onPageChange: setPage, toolbarProps: {
            showFiltros: true,
            filtros,
            rangoDefault,
            opcionesEstado: [],
...
export default TransaccionNoCuadrada;
