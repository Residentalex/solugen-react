...
const ChatWidget = () => {
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e) => {
            ...
            const newRight = Math.max(20, Math.min(dragOrigin.current.right - deltaX, window.innerWidth - 68));
            const newBottom = Math.max(20, Math.min(dragOrigin.current.bottom - deltaY, window.innerHeight - 68));
    ...
    // --- fin arrastre ---
    if (viewState === 'closed') {
        return (_jsxs(_Fragment, { children: [noLeidos > 0 && (_jsx("div", { style: {
                    ...
                    }, children: noLeidos })), _jsx("button", { onClick: abrir, title: "Chat", style: {
                    ...
                    }, onMouseEnter: (e) => (e.currentTarget.style.transform = 'scale(1.1)'), onMouseLeave: (e) => (e.currentTarget.style.transform = 'scale(1)'), onMouseDown: (e) => {
                        if (viewState !== 'closed')
                            return;
                        e.preventDefault();
    ...
    return (_jsxs("div", { ref: widgetRef, style: {
        ...
        }, children: [viewState === 'list' && (_jsx(ChatConversationList, { onSelectConversacion: seleccionarConversacion })), viewState === 'chat' && (_jsx(ChatMessages, { onBack: volverALista }))] }));
};
