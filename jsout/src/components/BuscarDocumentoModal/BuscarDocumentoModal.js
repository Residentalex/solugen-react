...
export const normalizarOrigen = (val) => {
    if (val == null)
    ...
    if (s === 'debito' || s === 'd' || s === '0')
        return OrigenCuenta.Debito;
    if (s === 'credito' || s === 'c' || s === '1')
        return OrigenCuenta.Credito;
    return OrigenCuenta.Desconocido;
};
const BuscarDocumentoModal = ({ open, onClose, onSelect, tipoEntidad, codEntidad, origen, esDocumentoInventario, montoTotal, documentosIniciales, documentoEnviado, puedeAsignar = true, }) => {
    const TIPO_DOC_CODES = [
    ...
    const obtenerCodigoCompleto = useCallback((doc) => {
    ...
    const _obtenerPendienteReal = useCallback((doc) => {
    ...
    const calcularPendiente = useCallback((doc) => {
    ...
    const cargar = useCallback(async () => {
        ...
        try {
            ...
            const params = {};
            if (!esDocumentoInventario)
                params.tipoEntidad = tipoEntidad;
            const { data: respData } = await apiClient.get(endpoint, { params });
            let docs = respData?.data || [];
            // ===== Filtrar documentos con pendiente real != 0 (tanto saldo como sobrepago) =====
            docs = docs.filter((d) => _obtenerPendienteReal(d) !== 0);
            ...
            if (!esDocumentoInventario && origen !== undefined && origen !== null) {
                docs = docs.filter((d) => {
                    ...
                    if (docOrigen === OrigenCuenta.Desconocido)
                        return true;
            ...
            if (documentoEnviado) {
                docs = docs.filter((d) => obtenerCodigoCompleto(d) !== documentoEnviado);
            }
            setDocumentos(docs);
            return docs;
        }
    ...
    useEffect(() => {
        if (open) {
            ...
            cargar().then((docs) => {
                ...
                setDistribuido(Object.values(montosIniciales).reduce((s, v) => s + (v || 0), 0));
    ...
    const handleSelectionChange = (keys) => {
        ...
        const totalNuevo = keys.reduce((s, id) => {
    ...
    const columnas = [
        ...
        {
            title: 'Saldo Pendiente', key: 'saldo', width: 120, align: 'right',
            render: (_, r) => {
                const pendienteReal = _obtenerPendienteReal(r);
                return _jsx("strong", { children: formatNumber(Math.abs(pendienteReal)) });
            },
        },
        {
            ...
            render: (_, record) => {
                ...
                return (_jsx(InputNumber, { size: "small", style: { width: '100%' }, min: 0, max: maxMonto, step: 0.01, precision: 2, controls: false, disabled: !puedeAsignar || !estaSeleccionado, value: montosPorFila[String(record.id)] ?? (estaSeleccionado ? maxMonto : 0), onChange: (val) => {
    ...
    const totalDistribuido = selectedRowKeys.reduce((s, k) => s + (montosPorFila[String(k)] || 0), 0);
    return (_jsx(Modal, { title: "Buscar Documentos Relacionados", open: open, onCancel: onClose, footer: _jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }, children: [_jsxs(Space, { children: [_jsx("span", { children: "Monto a distribuir:" }), _jsx(InputNumber, { size: "small", style: { width: 120 }, min: 0, step: 0.01, precision: 2, value: montoADistribuir, onChange: (val) => setMontoADistribuir(val || 0) }), _jsxs("span", { style: { marginLeft: 16, color: '#888' }, children: ["Distribuido: ", formatNumber(totalDistribuido), " |", ' ', "Disponible:", ' ', _jsx("span", { style: { color: (montoADistribuir - totalDistribuido) < 0 ? '#ff4d4f' : '#52c41a' }, children: formatNumber(Math.max(0, montoADistribuir - totalDistribuido)) })] })] }), _jsxs(Space, { children: [_jsx(Button, { onClick: handleAsignar, children: "Asignar" }), _jsx(Button, { onClick: onClose, children: "Cancelar" }), _jsxs(Button, { type: "primary", onClick: handleConfirm, disabled: selectedRowKeys.length === 0, children: ["Agregar (", selectedRowKeys.length, ")"] })] })] }), width: 1000, destroyOnHidden: true, children: _jsx(Table, { dataSource: documentos, columns: columnas, rowKey: (r) => r.id, loading: loading, size: "small", pagination: { pageSize: 10, showSizeChanger: false }, rowSelection: {
                selectedRowKeys,
