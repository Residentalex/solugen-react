import { jsx as _jsx, jsxs as _jsxs, Fragment as _Fragment } from "react/jsx-runtime";
import React from 'react';
import { Modal, Input, Tag, Typography, Spin } from 'antd';
import { SearchOutlined, CloseOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import BuscadorGlobalSeccion from './BuscadorGlobalSeccion';
import BuscadorGlobalResultado from './BuscadorGlobalResultado';
import { productoApi } from '../../api/productoApi';
import { transaccionApi } from '../../api/transaccionApi';
import { entidadApi } from '../../api/entidadApi';
import { useAuthStore } from '../../stores/authStore';
const { Text } = Typography;
const ICONOS_SECCION = {
    productos: 'ðŸ“¦',
    clientes: 'ðŸ‘¤',
    proveedores: 'ðŸ­',
    entidades: 'ðŸ¢',
    documentos: 'ðŸ“„',
};
const CHIPS_ACCESO_RAPIDO = [
    { label: 'ðŸ·ï¸  Productos', key: 'productos' },
    { label: 'ðŸ‘¤  Clientes', key: 'MCliente' },
    { label: 'ðŸ¢  Entidades', key: 'entidades' },
    { label: 'ðŸ“„  Documentos', key: 'documentos' },
    { label: '🔗  Módulos', key: '' },
];
const PREFIJOS_DOC = ['ENP', 'SAP', 'FAC', 'PV', 'RDE', 'ORC', 'DVC', 'TRP', 'DEV', 'COTV', 'NCSUP', 'NCCLI', 'NDSUP', 'NDCLI', 'DBA', 'RI', 'SPA'];
const DOC_PREFIX_ROUTE = {
    ENP: 'FENP', // Entrada de Almacén
    SAP: 'FSAP', // Salida de Almacén
    FAC: 'FFAC', // Factura Cliente
    PV: 'FPV', // Factura POS
    RDE: 'FRDE', // Factura Suplidor
    ORC: 'FORC', // Orden de Compra
    DVC: 'FDVC', // Devolución de Compra
    TRP: 'FTRP', // Transferencia de Almacén
    DEV: 'FDEV', // Devolución de Venta
    COTV: 'FCotizacion', // Cotizaciones
    NCSUP: 'FNCSUP', // Nota Crédito CXP
    NCCLI: 'FNCCLI', // Nota Crédito CXC
    NDSUP: 'FNDSUP', // Nota Débito CXP
    NDCLI: 'FNDCLI', // Nota Débito CXC
    DBA: 'FDBASUP', // Distribución Balance
    RI: 'FRI', // Recibo Ingreso
    SPA: 'FSPA', // Solicitud de Pago
};
const BuscadorGlobalModal = ({ open, onClose }) => {
    const navigate = useNavigate();
    const sucursalesPermitidas = useAuthStore((s) => s.sucursalesPermitidas);
    const setSucursalActiva = useAuthStore((s) => s.setSucursalActiva);
    const usuario = useAuthStore((s) => s.usuario);
    const [searchText, setSearchText] = React.useState('');
    const [activeIndex, setActiveIndex] = React.useState(-1);
    const [searching, setSearching] = React.useState(false);
    const [resultados, setResultados] = React.useState(null);
    const [searched, setSearched] = React.useState(false);
    const abortRef = React.useRef(null);
    const [filtrosActivos, setFiltrosActivos] = React.useState(new Set());
    // Resetear estado al cerrar
    React.useEffect(() => {
        if (!open) {
            setSearchText('');
            setActiveIndex(-1);
            setResultados(null);
            setSearched(false);
            setFiltrosActivos(new Set());
        }
    }, [open]);
    // Responsive width
    const [modalWidth, setModalWidth] = React.useState(640);
    const [hideBadge, setHideBadge] = React.useState(false);
    React.useEffect(() => {
        const handleResize = () => {
            const w = window.innerWidth;
            if (w <= 480) {
                setModalWidth('95vw');
                setHideBadge(true);
            }
            else if (w <= 768) {
                setModalWidth('90vw');
                setHideBadge(false);
            }
            else {
                setModalWidth(640);
                setHideBadge(false);
            }
        };
        handleResize();
        window.addEventListener('resize', handleResize);
        return () => window.removeEventListener('resize', handleResize);
    }, []);
    // Búsqueda real contra APIs con resultados incrementales y agrupados por entidad
    const handleSearch = React.useCallback(async (texto) => {
        const q = texto.trim().replace(/\*/g, '%');
        const tieneFiltros = filtrosActivos.size > 0;
        // Detectar prefijo de tipo de documento (ej: "enp-89603" → tipoDoc=ENP, valor=89603)
        let docPrefijoTipo = '';
        let docPrefijoValor = q;
        const prefijoMatch = q.match(/^([a-z]+)-(.*)/i);
        if (prefijoMatch) {
            const posiblePrefijo = prefijoMatch[1].toUpperCase();
            if (PREFIJOS_DOC.includes(posiblePrefijo)) {
                docPrefijoTipo = posiblePrefijo;
                docPrefijoValor = prefijoMatch[2].replace(/\*/g, '%');
            }
        }
        if (q.length < 2 && !tieneFiltros)
            return;
        // Cancelar búsqueda anterior
        if (abortRef.current)
            abortRef.current.abort();
        abortRef.current = new AbortController();
        setSearching(true);
        setActiveIndex(-1);
        setSearched(true);
        setResultados(null);
        const sucursales = sucursalesPermitidas.length > 0
            ? sucursalesPermitidas.map((s) => s.sucursal)
            : [4];
        const productosMap = new Map();
        const documentosMap = new Map();
        const entidadesMap = new Map();
        const promesas = [];
        // Determinar qué tipos buscar según filtros activos
        const buscarTodo = filtrosActivos.size === 0;
        const buscarProductos = buscarTodo || filtrosActivos.has('productos');
        const buscarDocumentos = buscarTodo || filtrosActivos.has('documentos');
        const buscarEntidades = buscarTodo || filtrosActivos.has('entidades');
        // Helper para convertir maps a record y actualizar estado (máx. 10 total)
        const actualizarResultados = () => {
            const r = {};
            let count = 0;
            const MAX = 10;
            const pArray = Array.from(productosMap.values());
            const dArray = Array.from(documentosMap.values());
            const eArray = Array.from(entidadesMap.values());
            if (pArray.length > 0 && count < MAX) {
                r.productos = pArray.slice(0, Math.min(pArray.length, MAX - count));
                count += r.productos.length;
            }
            if (dArray.length > 0 && count < MAX) {
                r.documentos = dArray.slice(0, Math.min(dArray.length, MAX - count));
                count += r.documentos.length;
            }
            if (eArray.length > 0 && count < MAX) {
                r.entidades = eArray.slice(0, Math.min(eArray.length, MAX - count));
            }
            setResultados(Object.keys(r).length > 0 ? r : null);
        };
        for (const suc of sucursales) {
            const sucNombre = sucursalesPermitidas.find(s => s.sucursal === suc)?.nombre || `Suc. ${suc}`;
            // --- PRODUCTOS (solo si no hay filtro o el filtro es productos) ---
            if (buscarProductos) {
                const agregarProducto = (item, origen) => {
                    const key = item.codigo;
                    const existing = productosMap.get(key);
                    if (existing) {
                        if (!existing.sucursales.includes(sucNombre))
                            existing.sucursales.push(sucNombre);
                    }
                    else {
                        const subtitulo = origen === 'codigo'
                            ? `Código: ${key}`
                            : origen === 'referencia'
                                ? `Ref: ${item.referencia || item.refFabricante || 'N/A'}`
                                : `Equiv: ${q}`;
                        productosMap.set(key, {
                            id: `prod-${key}`,
                            titulo: item.nombre,
                            subtitulo,
                            tag: `${(item.precio || 0).toLocaleString('es-DO')}`,
                            tagColor: 'green',
                            tipo: 'producto',
                            sucursales: [sucNombre],
                            sucursalCodigo: suc,
                        });
                    }
                };
                promesas.push(productoApi.buscarPorCampo(suc, 'codigo', q, 10)
                    .then(items => { items.forEach(i => agregarProducto(i, 'codigo')); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar producto por codigo', err)));
                promesas.push(productoApi.buscarPorCampo(suc, 'referencia', q, 10)
                    .then(items => { items.forEach(i => agregarProducto(i, 'referencia')); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar producto por referencia', err)));
                promesas.push(productoApi.buscarPorCampo(suc, 'equival', q, 10)
                    .then(items => { items.forEach(i => agregarProducto(i, 'equival')); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar producto por equival', err)));
            }
            // --- DOCUMENTOS (solo si no hay filtro o el filtro es documentos) ---
            if (buscarDocumentos) {
                const agregarDocumento = (item, origen) => {
                    const key = `${item.id}`;
                    const existing = documentosMap.get(key);
                    if (existing) {
                        if (!existing.sucursales.includes(sucNombre))
                            existing.sucursales.push(sucNombre);
                    }
                    else {
                        const subtitulo = origen === 'documento'
                            ? `${item.concepto || ''} · ${item.entidad || ''} · ${item.fecha ? new Date(item.fecha).toLocaleDateString('es-DO') : ''}`
                            : origen === 'ncf'
                                ? `NCF: ${item.ncf || 'N/A'} · ${item.concepto || ''}`
                                : `Doc Ref: ${item.referencia || 'N/A'} · ${item.entidad || ''}`;
                        documentosMap.set(key, {
                            id: `doc-${key}`,
                            titulo: item.documento,
                            subtitulo,
                            tag: `${(item.total || 0).toLocaleString('es-DO')}`,
                            tagColor: 'purple',
                            tipo: 'documento',
                            sucursales: [sucNombre],
                            sucursalCodigo: suc,
                        });
                    }
                };
                promesas.push(transaccionApi.buscarPorCampo(suc, 'documento', docPrefijoValor, 10, docPrefijoTipo || undefined)
                    .then(items => { items.forEach(i => agregarDocumento(i, 'documento')); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar documento', err)));
                promesas.push(transaccionApi.buscarPorCampo(suc, 'ncf', q, 10)
                    .then(items => { items.forEach(i => agregarDocumento(i, 'ncf')); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar por NCF', err)));
                promesas.push(transaccionApi.buscarPorCampo(suc, 'doc_ref', q, 10)
                    .then(items => { items.forEach(i => agregarDocumento(i, 'doc_ref')); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar por doc_ref', err)));
            }
            // --- ENTIDADES (solo si no hay filtro o el filtro es entidades) ---
            if (buscarEntidades) {
                const agregarEntidad = (item) => {
                    const key = item.codigo || item.id || `${Math.random()}`;
                    const existing = entidadesMap.get(key);
                    if (existing) {
                        if (!existing.sucursales.includes(sucNombre))
                            existing.sucursales.push(sucNombre);
                    }
                    else {
                        entidadesMap.set(key, {
                            id: `ent-${key}`,
                            titulo: item.nombre || '',
                            subtitulo: `RNC: ${item.identificacion || 'N/A'} · ${item.descripcion || item.entidad || ''}`,
                            tag: item.codigo || '',
                            tagColor: 'orange',
                            tipo: 'entidad',
                            sucursales: [sucNombre],
                            sucursalCodigo: suc,
                        });
                    }
                };
                promesas.push(entidadApi.buscar(suc, q, 10)
                    .then(items => { items.forEach(i => agregarEntidad(i)); actualizarResultados(); })
                    .catch((err) => console.warn('Error al buscar entidades', err)));
            }
        }
        await Promise.allSettled(promesas);
        // Limitar a máximo 10 resultados en total
        const MAX_TOTAL = 10;
        let totalCount = 0;
        const productosArray = Array.from(productosMap.values());
        const docsArray = Array.from(documentosMap.values());
        const entidadesArray = Array.from(entidadesMap.values());
        const finalR = {};
        if (productosArray.length > 0 && totalCount < MAX_TOTAL) {
            const take = Math.min(productosArray.length, MAX_TOTAL - totalCount);
            finalR.productos = productosArray.slice(0, take);
            totalCount += take;
        }
        if (docsArray.length > 0 && totalCount < MAX_TOTAL) {
            const take = Math.min(docsArray.length, MAX_TOTAL - totalCount);
            finalR.documentos = docsArray.slice(0, take);
            totalCount += take;
        }
        if (entidadesArray.length > 0 && totalCount < MAX_TOTAL) {
            const take = Math.min(entidadesArray.length, MAX_TOTAL - totalCount);
            finalR.entidades = entidadesArray.slice(0, take);
            totalCount += take;
        }
        setResultados(Object.keys(finalR).length > 0 ? finalR : null);
        setSearching(false);
    }, [sucursalesPermitidas, filtrosActivos]);
    // Lista plana para navegación con teclado
    const allResults = React.useMemo(() => {
        if (!resultados)
            return [];
        const items = [];
        for (const [seccion, lista] of Object.entries(resultados)) {
            for (const item of lista) {
                items.push({ id: item.id, section: seccion });
            }
        }
        return items;
    }, [resultados]);
    // Calcular contadores por sección
    const contadores = React.useMemo(() => {
        if (!resultados)
            return {};
        const c = {};
        for (const [seccion, items] of Object.entries(resultados)) {
            c[seccion] = items.length;
        }
        return c;
    }, [resultados]);
    const handleKeyDown = (e) => {
        if (e.key === 'ArrowDown') {
            e.preventDefault();
            setActiveIndex((prev) => Math.min(prev + 1, allResults.length - 1));
        }
        else if (e.key === 'ArrowUp') {
            e.preventDefault();
            setActiveIndex((prev) => Math.max(prev - 1, 0));
        }
        else if (e.key === 'Enter') {
            e.preventDefault();
            if (searchText.length >= 2 || filtrosActivos.size > 0) {
                handleSearch(searchText);
            }
        }
    };
    const handleItemClick = (item) => {
        // Validar permisos: ¿tiene acceso a la pantalla?
        const tieneAcceso = (codigoPantalla) => {
            return usuario?.pantallas?.some((p) => p.codigo?.toUpperCase() === codigoPantalla?.toUpperCase()) ?? false;
        };
        // Cambiar a la sucursal donde se encontró
        if (item.sucursalCodigo !== undefined) {
            setSucursalActiva(item.sucursalCodigo);
        }
        if (item.tipo === 'producto') {
            if (tieneAcceso('MProducto')) {
                navigate(`/MProducto/${item.codigoReal || ''}`);
            }
        }
        else if (item.tipo === 'cliente') {
            if (tieneAcceso('MCliente')) {
                navigate('/MCliente');
            }
        }
        else if (item.tipo === 'proveedor') {
            if (tieneAcceso('MProveedor')) {
                navigate('/MProveedor');
            }
        }
        else if (item.tipo === 'documento') {
            const prefijo = (item.titulo || '').split('-')[0];
            const ruta = DOC_PREFIX_ROUTE[prefijo] || 'FENP';
            if (tieneAcceso(ruta)) {
                const transacid = (item.id || '').replace('doc-', '');
                navigate(`/${ruta}/${transacid}`);
            }
        }
        else if (item.tipo === 'entidad') {
            if (tieneAcceso('MCliente')) {
                const codigoEntidad = (item.id || '').replace('ent-', '');
                navigate(`/MCliente/${codigoEntidad}`);
            }
        }
        onClose();
    };
    const handleClear = () => {
        setSearchText('');
        setActiveIndex(-1);
    };
    return (_jsxs(Modal, { open: open, onCancel: onClose, footer: null, width: modalWidth, centered: true, closable: false, destroyOnHidden: true, className: "paces-search-modal", style: { top: 80 }, children: [_jsx(Input, { placeholder: "\u00BFQu\u00E9 est\u00E1s buscando?", size: "large", variant: "borderless", prefix: _jsx(SearchOutlined, { style: { color: 'var(--paces-text-secondary)', fontSize: 20 } }), suffix: searchText.length > 0 ? (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 2 }, children: [_jsx(SearchOutlined, { style: { color: 'var(--paces-text-secondary)', fontSize: 16, cursor: 'pointer' }, onClick: () => handleSearch(searchText) }), _jsx(CloseOutlined, { style: { color: 'var(--paces-text-secondary)', fontSize: 16, cursor: 'pointer' }, onClick: handleClear })] })) : !hideBadge ? (_jsx("kbd", { style: {
                        background: 'var(--paces-bg-layout)',
                        color: 'var(--paces-text-secondary)',
                        fontSize: 11,
                        padding: '2px 8px',
                        borderRadius: 4,
                        fontFamily: 'monospace',
                    }, children: "Ctrl+K" })) : null, style: {
                    height: 56,
                    fontSize: 16,
                    borderRadius: 12,
                    paddingLeft: 16,
                    background: 'var(--paces-topbar-search-bg)',
                }, autoFocus: true, value: searchText, onChange: (e) => {
                    setSearchText(e.target.value);
                    setActiveIndex(-1);
                }, onKeyDown: handleKeyDown }), filtrosActivos.size > 0 && (_jsxs("div", { style: { marginTop: 16, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }, children: [Array.from(filtrosActivos).map((f) => (_jsx(Tag, { closable: true, onClose: () => {
                            setFiltrosActivos((prev) => { const n = new Set(prev); n.delete(f); return n; });
                            setResultados(null);
                            setSearched(false);
                        }, style: { borderRadius: 6 }, children: f === 'productos' ? 'Productos' : f === 'entidades' ? 'Entidades' : 'Documentos' }, f))), _jsx(Text, { type: "secondary", style: { fontSize: 12 }, children: filtrosActivos.size === 1 ? 'Filtro activo. Escribe y presiona Enter para buscar.' : 'Filtros activos. Escribe y presiona Enter para buscar.' })] })), searchText.length === 0 && !searched && (_jsxs(_Fragment, { children: [_jsx("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 20 }, children: CHIPS_ACCESO_RAPIDO.map((item) => (_jsx(Tag, { style: {
                                cursor: 'pointer',
                                padding: '4px 16px',
                                borderRadius: 8,
                                fontSize: 13,
                                margin: 0,
                                ...(filtrosActivos.has(item.key) ? {
                                    background: 'var(--paces-primary)',
                                    color: '#fff',
                                } : {}),
                            }, onClick: () => {
                                // Toggle filtro sin ejecutar búsqueda
                                if (item.key === 'productos' || item.key === 'entidades' || item.key === 'documentos') {
                                    setFiltrosActivos((prev) => {
                                        const next = new Set(prev);
                                        if (next.has(item.key))
                                            next.delete(item.key);
                                        else
                                            next.add(item.key);
                                        return next;
                                    });
                                }
                                else if (item.key === 'MCliente') {
                                    navigate('/MCliente');
                                    onClose();
                                }
                                else {
                                    onClose();
                                }
                            }, children: item.label }, item.label))) }), _jsx("div", { style: { marginTop: 16 }, children: _jsx(Text, { type: "secondary", style: { fontSize: 12 }, children: "Presiona Ctrl+K para abrir en cualquier momento" }) })] })), searchText.length > 0 && searchText.length < 2 && !searching && (_jsx("div", { style: { textAlign: 'center', padding: '40px 0' }, children: _jsx(Text, { type: "secondary", children: "Escribe al menos 2 caracteres..." }) })), searching && (_jsxs("div", { style: { textAlign: 'center', padding: '20px 0' }, children: [_jsx(Spin, { size: "small" }), _jsx("div", { style: { marginTop: 8 }, children: _jsxs(Text, { type: "secondary", style: { fontSize: 12 }, children: ["Buscando en ", sucursalesPermitidas.length || 1, " sucursal(es)..."] }) })] })), searched && !searching && !resultados && (_jsxs("div", { style: { textAlign: 'center', padding: '40px 0' }, children: [_jsx("div", { style: { fontSize: 48, marginBottom: 12 }, children: "\u00F0\u0178\u201D\u008D" }), _jsxs(Text, { type: "secondary", children: ["Sin resultados para \"", searchText, "\""] }), _jsx("br", {}), _jsx(Text, { type: "secondary", style: { fontSize: 12 }, children: "Revisa la ortograf\u00EDa o prueba con t\u00E9rminos m\u00E1s generales" })] })), resultados &&
                Object.entries(resultados).map(([seccion, items]) => (_jsx(BuscadorGlobalSeccion, { icono: ICONOS_SECCION[seccion] || 'ðŸ“„', nombre: seccion.charAt(0).toUpperCase() + seccion.slice(1), contador: contadores[seccion] || 0, children: items.map((item) => {
                        const globalIdx = allResults.findIndex((r) => r.id === item.id);
                        return (_jsx("div", { "data-index": globalIdx, children: _jsx(BuscadorGlobalResultado, { icono: ICONOS_SECCION[seccion] || 'ðŸ“„', titulo: item.titulo, subtitulo: item.subtitulo, tag: item.tag, tagColor: item.tagColor, isActive: activeIndex === globalIdx, onClick: () => handleItemClick(item), sucursales: item.sucursales }) }, item.id));
                    }) }, seccion)))] }));
};
export default BuscadorGlobalModal;
