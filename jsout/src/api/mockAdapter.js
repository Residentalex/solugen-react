...
function crearModulosPantallas() {
    const modulos = [
...
function generarFechaYYYYMMDD(diasAtras) {
...
function facturaSuplidorMock(i) {
...
function detalleArticulos() {
    ...
    for (let i = 0; i < count; i++) {
        let idx;
        do {
            idx = Math.floor(Math.random() * articulosBase.length);
        } while (used.has(idx));
        used.add(idx);
...
function asientosContables(montoTotal) {
    return [
        {
            id: 1,
...
function entradaDetalleMock(id) {
...
function detalleGenerico(id, articulos, total, tipoDoc, nombreDoc) {
    const monedaDefault = getMonedaSucursalActiva();
    return {
        id,
...
function generarLista(generator, count) {
...
export function setupMocks() {
    ...
    const okVoid = [200, { isSuccess: true, data: null, errorMessage: '' }];
    function handleList(data) {
        return okResp(data);
    }
    function handleDetail(url, generator, defaultData) {
    ...
    mock.onGet(new RegExp('/api/Rol/\\d+/pantallas-disponibles')).reply(() => {
        ...
        const data = pantallas.map((p) => ({
            pantalla: { ...p, acciones: [], esReporte: false, pantallaPadreID: undefined },
    ...
    mock.onPost(new RegExp('/api/Usuario/\\d+$')).reply((c) => {
        ...
        const newUser = {
            ...
            roles: (body.roles || []).map((r) => ({ id: r.rolID, nombre: 'Rol' })),
            sucursalesRoles: [],
            pantallas: [],
            permisosEspeciales: [],
        };
    ...
    mock.onPut(new RegExp('/api/Usuario/\\d+/\\d+/estado')).reply((c) => {
        ...
        if (idx >= 0)
            USUARIOS[idx].activo = body.activo;
        return okVoid;
    });
    ...
    mock.onGet(new RegExp('/api/Producto/\\d+$')).reply((c) => {
        ...
        if (activo === 'true')
            ...
            filtered = filtered.filter((p) => !p.activo);
        if (codigo)
            filtered = filtered.filter((p) => p.codigo.includes(codigo) || p.nombre.toLowerCase().includes(codigo.toLowerCase()));
    ...
    mock.onGet(new RegExp('/api/Producto/\\d+/filtrar')).reply((c) => {
        ...
        let filtered = [...PRODUCTOS];
        if (codigo)
            filtered = filtered.filter((p) => p.codigo.includes(codigo));
        if (referencia)
            filtered = filtered.filter((p) => p.referencia?.toLowerCase().includes(referencia.toLowerCase()));
        return okResp(filtered);
    });
    mock.onGet(new RegExp('/api/Producto/\\d+/[^/]+$')).reply((c) => {
