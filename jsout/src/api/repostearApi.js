...
export const repostearApi = {
    ...
    tieneRutaEspecifica: (tipoDoc) => {
        return tipoDoc in RUTAS;
    },
    /** Iniciar reposteo por rango de fechas. Devuelve jobId para suscribirse al progreso via SignalR */
    repostear: async (tipoDoc, sucursal, fechaDesde, fechaHasta, destino) => {
        const ruta = RUTAS[tipoDoc];
        if (!ruta)
            throw new Error(`Tipo de documento no soportado para reposteo: ${tipoDoc}`);
        ...
        const { data } = await apiClient.put(`${ruta}/${sucursal}/repostear`, null, { params });
        if (!data.isSuccess) {
