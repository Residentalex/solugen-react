/**
 * Genera los 8 archivos golden .raw.txt usando el formatter ACTUAL (no el bundle viejo).
 */
import { createHash } from 'node:crypto';
import { writeFileSync } from 'node:fs';
import { join, resolve } from 'node:path';

// Import directo del codigo fuente actual
import { formatTicketPOS, formatTicketReciboIngreso } from '../src/utils/escpos-formatter';
...
console.log('=== GOLDENS REGENERADOS (formatter actual) ===');
for (const c of casos) {
  writeFileSync(join(dir, c.nombre + '.raw.txt'), c.salida, 'latin1');
  const h = sha(c.salida);
  console.log(`${c.nombre}: ${c.salida.length}c | SHA256: ${h}`);
}
