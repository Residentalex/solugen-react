...
/** Contexto de datos disponible para las expresiones. */
export interface ContextoDatos {
  /** DTO del documento (factura, turno, recibo, voucher...). */
  data?: any;
  /** Informacion de la compania (nombre, direccion, telefono, rnc...). */
  company?: any;
  /** La propia configuracion de plantilla (permite referenciar otros campos). */
  config?: any;
  /** Item iterado actual (tiene prioridad maxima al resolver campos). */
  item?: any;
}

export type CategoriaFuncion =
  | 'agregadas'
  | 'logica'
  | 'matematicas'
  | 'texto'
...
/** Metadatos de una funcion de la whitelist (para ayuda y validacion). */
export interface FuncionDef {
  nombre: string;
  categoria: CategoriaFuncion;
  params: string;
  descripcion: string;
  ejemplo: string;
}

/** Error de validacion de una expresion dentro de la plantilla. */
...
type Nodo =
  ...
  | { k: 'bool'; v: boolean }
  | { k: 'nul' }
  | { k: 'id'; segs: string[]; pos: number }
  | { k: 'un'; op: '-' | 'NOT'; a: Nodo }
  | { k: 'bin'; op: string; a: Nodo; b: Nodo }
  | { k: 'llam'; nombre: string; args: Nodo[]; pos: number };

class ParseError extends Error {
  readonly pos: number;
  constructor(mensaje: string, pos: number) {
    super(mensaje);
    this.pos = pos;
  }
}
...
function tokenizar(src: string): Token[] {
  ...
  const esLetra = (c: string) => /[A-Za-zÑñÁÉÍÓÚáéíóúÜü_]/.test(c);
  const esAlnum = (c: string) => esLetra(c) || esDigito(c);
...
class Parser {
  ...
  private esperarOp(v: string): void {
    const t = this.peek();
    if (t.t === 'op' && t.v === v) { this.avanzar(); return; }
...
function parseExpresion(src: string): Nodo {
...
function buscarProp(fuente: any, nombre: string): any {
  if (fuente === null || fuente === undefined || typeof fuente !== 'object') return undefined;
 ...
 */
function bajarCampos(base: any, segs: string[]): any {
  let v = base;
  for (const s of segs) {
 ...
 */
function resolverPath(segs: string[], ctx: ContextoDatos): any {
...
export function aString(v: any): string {
...
function aplicarOp(op: string, a: any, b: any): boolean {
  switch (op) {
    case '=': return a === b;
    case '<>': return a !== b;
    case '<': return a < b;
...
function datediff(faVal: any, fbVal: any, unidadRaw: any): number | null {
  ...
  if (['HORAS', 'HOUR', 'HOURS'].includes(u)) return Math.round(ms / 3600000);
  if (['MINUTOS', 'MINUTE', 'MINUTES'].includes(u)) return Math.round(ms / 60000);
  if (['DIAS', 'DAY', 'DAYS'].includes(u)) return Math.round(ms / 86400000);
  ...
  if (['ANIOS', 'AÑOS', 'YEAR', 'YEARS'].includes(u)) return Math.floor(meses / 12);
  return null;
}
...
function reqArgs(args: any[], min: number, nombre: string): void {
...
function numerosDe(v: any): number[] {
  const fuente = Array.isArray(v) ? v : [v];
  const out: number[] = [];
  for (const it of fuente) {
    if (it === null || it === undefined || typeof it === 'boolean') continue;
    const n = aNumero(it);
...
function extremos(args: any[], esMin: boolean): number | null {
  let pool: number[];
...
type ImplFn = (args: any[], ctx: ContextoDatos, nodos?: Nodo[]) => any;

/** Implementaciones de la whitelist. Cualquier otra funcion esta prohibida. */
const IMPL: Record<string, ImplFn> = {
  // ----- Agregadas -----
...
export const FUNCIONES: FuncionDef[] = [
  // ----- Agregadas -----
...
function evaluar(n: Nodo, ctx: ContextoDatos): any {
  switch (n.k) {
    case 'num':
    case 'str':
    case 'bool':
...
function evaluarBin(n: Extract<Nodo, { k: 'bin' }>, ctx: ContextoDatos): any {
  ...
  switch (n.op) {
    ...
    case '%': {
      const na = aNumero(a);
      const nb = aNumero(b);
      if (na === null || nb === null || nb === 0) return null; // division por cero -> null
      return n.op === '/' ? na / nb : na % nb;
    }
    default:
      return null;
...
export function evaluarExpresion(expresion: string, ctx: ContextoDatos = {}): any {
...
export function evaluarObjeto<T>(obj: T, ctx: ContextoDatos = {}): T {
  try {
    return recorrer(obj, ctx) as T;
  } catch {
    return obj;
  }
}

function recorrer(v: any, ctx: ContextoDatos): any {
...
export function validarExpresionesDeConfig(config: any): ResultadoValidacion[] {
  ...
  const revisar = (texto?: string | null): void => {
    if (!texto || typeof texto !== 'string' || texto.indexOf('{') === -1) return;
    for (const expr of extraerExpresiones(texto)) {
      const r = validarExpresion(expr);
      if (!r.ok) errores.push({ expresion: expr, error: r.error ?? 'Error desconocido' });
    }
  };
  ...
  if (tl && typeof tl === 'object') {
    for (const clave of Object.keys(tl)) {
      const v = tl[clave];
      if (typeof v === 'string') revisar(v);
      else if (v && typeof v === 'object') revisar(v.texto);
    }
  }
...
export function listarFunciones(): FuncionDef[] {
