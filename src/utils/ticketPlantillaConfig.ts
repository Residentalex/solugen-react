/**
 * Configuracion por defecto de plantillas de tickets ESC/POS.
 * `PLANTILLA_CONFIG_DEFAULT` reproduce EXACTAMENTE el formato actual de los
 * formateadores (ancho 48, feed 4, pie "** GRACIAS POR SU COMPRA **", todo visible).
 */
import type {
  PlantillaConfig,
...
export const CAMPOS_TICKET_LABELS: Record<string, string> = {
  NCF: 'NCF',
  TIPO_COMP: 'TIPO COMP',
  CAJERO: 'CAJERO',
  CAJA: 'CAJA',
...
export const CAMPOS_TICKET_LABELS_VSNT: Record<string, string> = {
  COMPANIA: 'Compañía',
  SUCURSAL: 'Sucursal',
  DIRECCION: 'Dirección',
  TELEFONO: 'Teléfono',
...
export const CAMPOS_DETALLE_LABELS: Record<string, string> = {
  CODIGO: 'Código',
  ARTICULO: 'Artículo',
  CANTIDAD: 'Cantidad',
  PRECIO: 'Precio',
...
export const CAMPOS_DETALLE_RI_LABELS: Record<string, string> = {
  DOCUMENTO: 'Documento',
 ...
 */
export const PLANTILLA_CONFIG_DEFAULT_RI: PlantillaConfig = {
 ...
 */
export const PLANTILLA_CONFIG_DEFAULT_VSNT: PlantillaConfig = {
  encabezado: {
    mostrarCompania: true,
    mostrarDireccion: true,
    mostrarTelefono: true,
 ...
 */
export const PLANTILLA_CONFIG_DEFAULT_VSNT_ANULACION: PlantillaConfig = {
 ...
 */
export const PLANTILLA_CONFIG_DEFAULT_VSNT_CIERRE: PlantillaConfig = {
  encabezado: {
    mostrarCompania: true,
    mostrarDireccion: true,
    mostrarTelefono: true,
 ...
 */
export function normalizarTextosLibres(
  textos: Record<string, string | TextoLibreConfig> | undefined,
): Record<string, TextoLibreConfig> | undefined {
  if (!textos) return undefined;
  const out: Record<string, TextoLibreConfig> = {};
...
function mergeConfig(base: PlantillaConfig, config: PlantillaConfig): PlantillaConfig {
  const out: PlantillaConfig = { ...base };
...
function esItemOrdenRI(item: string): boolean {
  return (
    CAMPOS_TICKET_ORDEN_RI.includes(item)
    || item === 'ESPACIO'
    || item === 'SEPARADOR'
 ...
 */
export function normalizarConfigRI(config?: PlantillaConfig | null): PlantillaConfig {
 ...
 */
export function normalizarConfigVSNT(config?: PlantillaConfig | null): PlantillaConfig {
 ...
 */
export function normalizarConfigVSNT_ANULACION(config?: PlantillaConfig | null): PlantillaConfig {
  if (!config) return { ...PLANTILLA_CONFIG_DEFAULT_VSNT_ANULACION, zonas: [...ZONAS_DEFAULT_VSNT_ANULACION] };
  const out = mergeConfig(PLANTILLA_CONFIG_DEFAULT_VSNT_ANULACION, config);
  return finalizarNormalizacionVSNT(out, config, ZONAS_DEFAULT_VSNT_ANULACION);
}

/**
 * Normaliza una config guardada contra el default de Voucher Visanet - Cierre.
 */
export function normalizarConfigVSNT_CIERRE(config?: PlantillaConfig | null): PlantillaConfig {
  if (!config) return { ...PLANTILLA_CONFIG_DEFAULT_VSNT_CIERRE, zonas: [...ZONAS_DEFAULT_VSNT_CIERRE] };
  const out = mergeConfig(PLANTILLA_CONFIG_DEFAULT_VSNT_CIERRE, config);
  return finalizarNormalizacionVSNT(out, config, ZONAS_DEFAULT_VSNT_CIERRE);
}

/** Helper comun a todos los normalizadores VSNT (zonas + textos + DTO + firmas + logo). */
function finalizarNormalizacionVSNT(
  out: PlantillaConfig,
  config: PlantillaConfig,
  zonasDefault: ZonaTicketConfig[],
): PlantillaConfig {
  if (config.columnasDetalle && !config.detalle?.columnas) {
    out.detalle = { ...(out.detalle || {}), columnas: { ...config.columnasDetalle } };
  }
...
// ===== Catalogo de campos DTO (base de datos) disponibles en el editor =====

/** Item del catalogo de campos DTO seleccionables en el editor. */
export interface CampoDTODisponible extends PlantillaCamposDTOConfig {
  /** Id interno del catalogo (no se guarda; el id guardado es dto_<timestamp>). */
  id: string;
}
 ...
 */
export const CAMPOS_DTO_DISPONIBLES: Record<'FPV' | 'FRI' | 'VSNT', CampoDTODisponible[]> = {
  FPV: [
 ...
 */
export const ZONAS_DEFAULT_FPV: ZonaTicketConfig[] = [
  {
 ...
 */
export const ZONAS_DEFAULT_FRI: ZonaTicketConfig[] = [
  {
 ...
 */
export const ZONAS_DEFAULT_VSNT: ZonaTicketConfig[] = [
  {
 ...
 */
export const ZONAS_DEFAULT_VSNT_ANULACION: ZonaTicketConfig[] = [
  {
 ...
 */
export const ZONAS_DEFAULT_VSNT_CIERRE: ZonaTicketConfig[] = [
  {
 ...
 */
export function migrarConfigAZonas(config: PlantillaConfig, tipo: 'FPV' | 'FRI' | 'VSNT'): ZonaTicketConfig[] {
  ...
  if (tipo === 'FPV') {
    ...
    const lineasCampos: LineaZonaConfig[] = [];
    for (const item of orden) {
      if (item === 'ESPACIO') { lineasCampos.push({ ref: 'ESPACIO' }); continue; }
      if (item === 'SEPARADOR') { lineasCampos.push({ ref: 'SEPARADOR' }); continue; }
      if (item.startsWith('LIBRE:')) { lineasCampos.push({ ref: item as LineaZonaConfig['ref'] }); continue; }
      if (item.startsWith('DTO:')) { lineasCampos.push({ ref: item as LineaZonaConfig['ref'] }); continue; }
      if (visibles[item] === false) continue;

      const ref = `CAMPO:${item}` as LineaZonaConfig['ref'];
      const linea: LineaZonaConfig = { ref };
      const lbl = labels[item];
      if (lbl !== undefined && lbl !== '' && lbl !== CAMPOS_TICKET_LABELS[item]) {
        linea.label = lbl;
      }
    ...
    const titLinea: LineaZonaConfig = { ref: 'CAMPO:TITULO' };
    ...
    // 5. Totales
    const lineasTot: LineaZonaConfig[] = [];
  ...
  const lineasTit: LineaZonaConfig[] = [];
  const titLinea: LineaZonaConfig = { ref: 'CAMPO:TITULO' };
  if (tit.texto) titLinea.label = tit.texto;
  if (tit.formato) titLinea.formato = { ...tit.formato };
  ...
  const ordenRI = (cam.orden && cam.orden.length > 0 && cam.orden.some((k) => esItemOrdenRI(k)))
    ? cam.orden
    : CAMPOS_TICKET_ORDEN_RI;
  const visRI = cam.visibles || {};
  const lblRI = cam.labels || {};
  const fmtRI = cam.formatos || {};

  const lineasCam: LineaZonaConfig[] = [];
  for (const item of ordenRI) {
    if (item === 'ESPACIO') { lineasCam.push({ ref: 'ESPACIO' }); continue; }
    if (item === 'SEPARADOR') { lineasCam.push({ ref: 'SEPARADOR' }); continue; }
    if (item.startsWith('LIBRE:')) { lineasCam.push({ ref: item as LineaZonaConfig['ref'] }); continue; }
    if (item.startsWith('DTO:')) { lineasCam.push({ ref: item as LineaZonaConfig['ref'] }); continue; }
    if (visRI[item] === false) continue;

    const ref = `CAMPO:${item}` as LineaZonaConfig['ref'];
    const linea: LineaZonaConfig = { ref };
    const lbl = lblRI[item];
    if (lbl !== undefined && lbl !== '' && lbl !== CAMPOS_TICKET_LABELS_RI[item]) {
      linea.label = lbl;
    }
