...
class NotificationClientService {
  ...
  async connect(usuarioID: number): Promise<void> {
    ...
    connection.on('NuevaNotificacion', (notificacion: NotificacionVista) => {
   ...
   */
  private async showBrowserNotification(titulo: string, opciones?: NotificationOptions): Promise<void> {
  ...
  private handleNuevaNotificacion(notificacion: NotificacionVista): void {
