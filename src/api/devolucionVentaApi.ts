...
import type { DevolucionVentaDTO, FiltroDevVenta } from '../types/devolucionVenta';
...
export const devolucionVentaApi = {
  obtenerVista: async (
    ...
    const { data } = await apiClient.get<ApiResponse<DevolucionVentaDTO[]>>(`${BASE}/${sucursal}`, { params });
    return data.data;
  },

  filtrar: async (
    sucursal: number,
    filtro: FiltroDevVenta
  ): Promise<DevolucionVentaDTO[]> => {
    const params: Record<string, string | number> = {};
    ...
    if (filtro.cliente) filtro.cliente && (params.cliente = filtro.cliente);
    if (filtro.factura) filtro.factura && (params.factura = filtro.factura);
    if (filtro.almacen) params.almacen = filtro.almacen;

    const { data } = await apiClient.get<ApiResponse<DevolucionVentaDTO[]>>(`${BASE}/${sucursal}/filtrar`, { params });
    return data.data;
  },
