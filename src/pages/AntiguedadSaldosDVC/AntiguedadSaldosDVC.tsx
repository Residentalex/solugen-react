...
import type { CompaniaDTO } from '../../types/entradaAlmacen';
...
interface BalanceProcesado {
  ...
  documento: { origenCuenta: string } | null;
}

function procesarBalancesDVC(items: any[]): BalanceProcesado[] {
  return items
    .map((item) => {
      // 1. Filtrar nulos de transacciones asociadas
      const transaccionesValidas = (item.transaccionesAsociadas || []).filter((x: any) => x != null);
      ...
      const debitos = transaccionesValidas.reduce((sum: number, t: any) => sum + (t.monto || 0), 0);
...
const AntiguedadSaldosDVC: React.FC = () => {
  ...
  const [nomTipo, setNomTipo] = useState<string>('');
  ...
  const [tiposOrig, setTiposOrig] = useState<any[]>([]);
  ...
  const generarReporte = useCallback(async () => {
    ...
    try {
      ...
      const resultados = await antiguedadSaldosDVCApi.obtenerBalances(
        sucursalActiva,
        hasta,
      ...
      setData(procesados as unknown as TransaccionBalanceDTO[]);
      setSearchText('');
      setPage(1);
  ...
  const handlePrint = async () => {
    ...
    try {
      const hasta = formatDateParam(fechaHasta.toDate());
      const blob = await antiguedadSaldosDVCApi.generarPDF(
        sucursalActiva, hasta,
  ...
  const buscarTipo = (valor: string) => {
    setSearchTipo(valor);
    if (!valor) {
      setTipos(tiposOrig);
      return;
    }
    const filtradas = tiposOrig.filter(
      (t) =>
        t.nombre?.toLowerCase().includes(valor.toLowerCase()) ||
        t.codigo?.toLowerCase().includes(valor.toLowerCase()),
    );
    setTipos(filtradas);
  };

  const seleccionarTipo = (item: any) => {
  ...
  const exportarCSV = () => {
    ...
    if (detallado) {
      ...
      excelData = agingData.map((item) => ({
        [headersParaExportar[0]]: item.tipoDocumento
  ...
  return (
    <>
      ...
      <Card className="paces-card no-print" style={{ marginBottom: 16 }}>
        <div style={{ padding: '16px 24px' }}>
          <Row gutter={[16, 12]}>
            ...
            <Col xs={24} sm={12} md={5}>
              <div style={{ marginBottom: 4 }}>
                <Text type="secondary" style={{ fontSize: 12 }}>Tipo DVC</Text>
              </div>
              <Input
            ...
            <Col xs={24} sm={12} md={5}>
              <div style={{ marginBottom: 4 }}>
                <Text type="secondary" style={{ fontSize: 12 }}>Compañía/Sucursal</Text>
              </div>
              <Input
      ...
      <Modal
        title="Buscar Tipo DVC"
        open={modalTipoAbierto}
        onCancel={() => setModalTipoAbierto(false)}
        footer={null}
        width={500}
        destroyOnHidden
      >
      ...
      <Modal
        title="Buscar Compañía/Sucursal"
