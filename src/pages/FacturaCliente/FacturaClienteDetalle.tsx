...
const FacturaClienteDetalle: React.FC = () => {
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: 80 }}>
        ...
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/FFAC')}>
          Volver
        </Button>
      </div>
    );
  ...
  const handleAplicar = async () => {
    ...
    setSaving(true);
    try {
      await facturaClienteApi.aplicar(sucursalActiva, parseInt(id));
  ...
  const handlePostear = async () => {
    ...
    setSaving(true);
    try {
      await facturaClienteApi.postear(sucursalActiva, data as any);
        ...
        <Space>
          ...
          {data.estado === 0 && data.periodo !== 6 && (
            <PermissionGate accion="EDITAR">
              <Button type="primary" icon={<EditOutlined />} onClick={() => navigate(`/FFAC/${id}/editar`)}>Editar</Button>
            </PermissionGate>
          )}
