...
const NotaCredito: React.FC<NotaCreditoProps> = ({ tipoEntidad }) => {
  ...
  const cargarDatos = useCallback(async (pagina: number, filas: number, busqueda: string) => {
    ...
    try {
      ...
      if (busqueda.length > 2) {
        ...
        resultados = await notaCreditoApi.filtrar(sucursalActiva, tipoEntidad, filtro);
      } else {
        resultados = await notaCreditoApi.obtenerVista(
  ...
  }, [sucursalActiva, tipoEntidad, filtros.desde, filtros.hasta, filtros.estado]);
  ...
  const handleImprimir = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get(`/reportes/contabilidad/nota-credito/${sucursalActiva}/${selectedRow.id}`, {
        responseType: 'blob',
      });
      const blobUrl = URL.createObjectURL(res.data);
      setPdfPreview({ url: blobUrl, title: `NC-${selectedRow.documento}` });
    } catch {
  ...
  const handleRowClick = (record: TransaccionVistaDTO) => {
    ...
    if (editable) {
      setEditarCallback(() => navigate(`/${codigoPantalla}/${record.id}/editar`));
    } else {
      setEditarCallback(undefined);
    }
  };
  ...
  return (
    <>
      {loadingError && (
        <Alert
          message="Error al cargar notas crédito"
          type="error"
          showIcon
