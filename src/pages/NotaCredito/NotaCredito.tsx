  ...
  const cargarDatos = useCallback(async (pagina: number, filas: number, busqueda: string) => {
    ...
    try {
      ...
      if (busqueda.length > 2) {
        ...
        result = await notaCreditoApi.filtrar(sucursalActiva, tipoEntidad, filtro);
      } else {
        result = await notaCreditoApi.obtenerVista(
  ...
  useEffect(() => {
    if (selectedRow) {
      setImprimirCallback(async () => {
        try {
          const res = await apiClient.get(`/reportes/contabilidad/nota-credito/${selectedRow.codigoSucursal ? obtenerNombreEnumSucursal(selectedRow.codigoSucursal) : sucursalActiva}/${selectedRow.id}`, {
            responseType: 'blob',
          });
