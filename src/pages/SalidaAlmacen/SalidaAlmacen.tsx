...
const SalidaAlmacen: React.FC = () => {
  ...
  const handleImprimir = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get(`/reportes/inventario/salida/${sucursalActiva}/${selectedRow.id}`, {
        responseType: 'blob',
      });
      const blobUrl = URL.createObjectURL(res.data);
      setPdfPreview({ url: blobUrl, title: `SAP-${selectedRow.documento}` });
    } catch {
  ...
  const handleRowClick = (record: MovimientoVistaDTO) => {
    ...
    if (editable) {
      setEditarCallback(() => navigate(`/FSAP/${record.id}/editar`));
    } else {
      setEditarCallback(undefined);
    }
  };
  ...
  const columns: ColumnsType<MovimientoVistaDTO> = [
{
  ...
  render: (doc: string, record: MovimientoVistaDTO) => (
    <Text strong className="paces-doc-link" onClick={() => navigate(`/FSAP/${record.id}`)}>{doc}</Text>
  ),
},
{
  title: 'Fecha',
...
},
{
  ...
  return (
    <>
      {loadingError && (
        <Alert
          message="Error al cargar salidas de almacén"
          type="error"
          showIcon
      ...
      <div style={{ padding: '16px 24px 0' }}>
        <div style={{
          ...
          <PermissionGate accion="CREAR">
            <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/FSAP/nuevo')}>
              Nuevo
            </Button>
          </PermissionGate>
