...
const ReciboIngreso: React.FC = () => {
  ...
  const cargarDatos = useCallback(async (pagina: number, filas: number, busqueda: string) => {
    ...
    try {
      ...
      if (busqueda.length > 2) {
        ...
        resultados = await reciboIngresoApi.filtrar(sucursalActiva, filtro);
      } else {
        resultados = await reciboIngresoApi.obtenerVista(
          sucursalActiva,
          desde, hasta,
  ...
  const handleRowClick = (record: TransaccionVistaDTO) => {
    ...
    if (editable) {
      setEditarCallback(() => navigate(`/FRI/${record.id}/editar`));
    } else {
      setEditarCallback(undefined);
    }
  };
  ...
  return (
    <>
      {loadingError && (
        <Alert
          message="Error al cargar recibos de ingreso"
          type="error"
          showIcon
        ...
        <div style={{ padding: '16px 24px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            <PermissionGate accion="CREAR">
              <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/FRI/nuevo')}>
                Nuevo
              </Button>
            </PermissionGate>
