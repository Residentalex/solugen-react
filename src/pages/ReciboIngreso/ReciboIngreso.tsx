  ...
  useEffect(() => {
    if (selectedRow) {
      setImprimirCallback(async () => {
        try {
          const res = await apiClient.get(`/reportes/contabilidad/reciboIngreso/${selectedRow.codigoSucursal ? obtenerNombreEnumSucursal(selectedRow.codigoSucursal) : sucursalActiva}/${selectedRow.id}`, {
            responseType: 'blob',
          });
      ...
      render: (_: any, record: TransaccionVistaDTO) => (
        <Space size="small">
          <Tooltip title="Ver detalle">
            <Button type="text" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/FRI/${record.id}`)} />
          </Tooltip>
