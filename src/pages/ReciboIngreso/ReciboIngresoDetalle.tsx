...
const ReciboIngresoDetalle: React.FC = () => {
  ...
  const handleAplicar = async () => {
    ...
    setSaving(true);
    try {
      await reciboIngresoApi.aplicar(sucursalActiva, parseInt(id));
  ...
  const handlePostear = async () => {
    ...
    setSaving(true);
    try {
      await reciboIngresoApi.postear(sucursalActiva, data as any);
        ...
        <Space>
          ...
          {data.estado === 0 && data.periodo !== 6 && (
            <PermissionGate accion="EDITAR">
              <Button type="primary" icon={<EditOutlined />} onClick={() => navigate(`/FRI/${id}/editar`)}>Editar</Button>
            </PermissionGate>
          )}
          {data.estado === 0 && data.periodo !== 6 && (
