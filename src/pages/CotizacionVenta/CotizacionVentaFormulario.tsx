...
// ===== Componente TotalesCard (simplificado, sin cobros) =====
...
const CotizacionVentaFormulario: React.FC = () => {
  ...
  const handleAnular = async () => {
    ...
    setSaving(true);
    try {
      await cotizacionVentaApi.anular(sucursalActiva, parseInt(id));
  ...
  const cargarConceptos = useCallback(async () => {
    try {
      const res = await facturaPOSApi.obtenerConceptos(sucursalActiva, 'COT');
  ...
  if (loading) {
    return (
      <div style={{ textAlign: 'center', padding: 80 }}>
        <Spin size="large" />
        <div style={{ marginTop: 16 }} className="paces-text-secondary">Cargando cotización...</div>
      </div>
    );
  }
