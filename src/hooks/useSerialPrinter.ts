...
// ===== Tipos locales para Web Serial API =====
// (evita dependencia de tipos DOM que pueden variar segun version TS)
...
export interface UseSerialPrinterReturn {
  /** Solicita al usuario que seleccione un puerto e inicia la conexion */
  connect: () => Promise<void>;
  /** Cierra el puerto serial */
  disconnect: () => Promise<void>;
  /** Envia texto + comandos ESC/POS de corte al puerto */
  print: (text: string) => Promise<void>;
  /** Indica si hay un puerto conectado y abierto */
  connected: boolean;
  /** Indica si hay una operacion de conexion en curso */
  connecting: boolean;
  /** Ultimo error ocurrido (null si no hay error) */
  error: string | null;
}
 ...
 */
export function useSerialPrinter(): UseSerialPrinterReturn {
  ...
  const obtenerSerial = useCallback((): SerialLike | null => {
    const nav = navigator as any;
    if (typeof nav.serial === 'undefined' || typeof nav.serial.requestPort !== 'function') {
      return null;
    }
    return nav.serial as SerialLike;
  }, []);
      ...
      for (const rate of baudRates) {
        try {
          ...
          const timeoutPromise = new Promise<void>((_, reject) =>
            setTimeout(() => reject(new Error(`Timeout (${rate} baud)`)), TIMEOUT_MS)
          );
          ...
          try { await port.close(); } catch { /* ignora error al cerrar tras fallo */ }
        }
      }
  ...
  const print = useCallback(async (text: string) => {
    ...
    try {
      ...
      let offset = 0;
      payload.set(CMD_INIT, offset); offset += CMD_INIT.length;
      payload.set(textBytes, offset); offset += textBytes.length;
      payload.set(CMD_FEED, offset); offset += CMD_FEED.length;
      ...
      const timeoutPromise = new Promise<void>((_, reject) =>
        setTimeout(() => reject(new Error('Timeout: la impresora no respondió en 10s')), TIMEOUT_MS)
      );
      ...
      try { writer.releaseLock(); } catch { /* ignora error al liberar lock */ }
    }
  }, []);
